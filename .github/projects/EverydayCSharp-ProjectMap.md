...
For rule-heavy features — nullable analysis, raw string literals, pattern precedence, and similar — enumerate the rules as a structured list (a short lead sentence, then bullets or a labeled example) and verify each rule against the language spec or reference *before* review. These features draw the most reviewer corrections when the rules are paraphrased loosely or stated from memory.
