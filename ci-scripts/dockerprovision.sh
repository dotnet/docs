...
docker run --name builder --rm constructors.azurecr.io/platforms/netcoresdk bash -c "for sample in $(find . -name *.csproj); do dotnet restore $sample; dotnet build $sample; done"
