...
<xref:Microsoft.Extensions.DataIngestion.IngestionChunkWriter`1> stores processed chunks into a data store for later retrieval. The library, which uses <xref:Microsoft.Extensions.AI> and <xref:Microsoft.Extensions.VectorData.Abstractions>, provides the <xref:Microsoft.Extensions.DataIngestion.VectorStoreWriter`1> class. This writer supports storing chunks in any [vector store](../vector-stores/overview.md) supported by <xref:Microsoft.Extensions.VectorData>.
