...
string endpoint = config["AZURE_OPENAI_ENDPOINT"] ?? throw new InvalidOperationException("Azure OpenAI endpoint not found");
string deployment = config["AZURE_OPENAI_DALLE_NAME"] ?? throw new InvalidOperationException("Azure OpenAI DALL-E deployment name not found");
