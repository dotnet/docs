...
When you use a `Dictionary`, connectors still need to know what the database schema looks like. Without the schema information, the connector would not be able to create a collection or know how to map to and from the storage representation that each database uses.
