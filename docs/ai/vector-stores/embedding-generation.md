---
title: Generate embeddings for vector store providers
description: Describes how you can generate embeddings to use with vector store providers.
...
Vector store providers support multiple ways of generating [embeddings](../conceptual/embeddings.md). You can generate them yourself and pass them as part of a record when using a <xref:Microsoft.Extensions.VectorData.VectorStoreCollection`2>. Or they can be generated internally to the <xref:Microsoft.Extensions.VectorData.VectorStoreCollection`2>.
...
To enable generating vectors automatically on upsert, the vector property on your data model is defined as the source type, for example, `string`, but is still decorated with a <xref:Microsoft.Extensions.VectorData.VectorStoreVectorAttribute>.
