...
One option discussed in Chapter 4 is a [direct HTTP call](service-to-service-communication.md#requestresponse-messaging) from the shopping basket to the catalog and pricing microservices. However, in chapter 4, we said synchronous HTTP calls *couple* microservices together, reducing their autonomy and diminishing their architectural benefits.
