...
Cloud native is about changing our thinking of how we construct and evolve critical business systems.
...
Applications have become increasingly complex with users demanding more and more. Users expect rapid responsiveness, innovative features, and zero downtime. Performance problems, recurring errors, and the inability to move fast are no longer acceptable. They'll simply move to your competitor.
...
As you can see, eBay, Netflix, and Uber expose systems that consist of hundreds of independent microservices. The architecture enables them to rapidly respond to market conditions. They can  instantaneously update small blocks of functionality without redeploying the entire system.
...
These capabilities are key to cloud-native applications. Let’s take some time to better understand the role of each.

## Born in the cloud…

Cloud-native systems are *all about the cloud*. They're designed and developed to thrive in a highly dynamic, virtualized cloud environment. They make extensive use of Platform as a Service (PaaS) compute infrastructure and managed services. They're built to exploit the cloud delivery model: elasticity, resilience, redundancy, orchestration, and on-demand features.  

Figure 1-3 shows a spectrum of compute services available in the Azure cloud. As you progress down the hierarchy, your focus shifts from managing infrastructure to business functionality. Cloud-native systems typically reside at these lower levels, using containers, PaaS, and serverless environments.
...
Many companies view the cloud and associated cloud-native systems as a competitive advantage – move fast, pivot quickly, and fail fast - while reducing operational and hardware costs.
...
Cloud-native systems embrace microservices, a popular architectural style for building modern applications. Microservices are constructed as a distributed set of small, independent services that interact through a shared fabric. They share the following characteristics: 
...
- Each service is self-contained. The domain logic, data, and state are encapsulated in the service. You select the data storage technology (SQL, NoSQL) and programming platform of choice.
...
Microservices provide agility. A set of related, but independent services, each with an autonomous lifecycle. Each can evolve independently and deploy frequently. You don’t have to wait for a quarterly release to deploy new features or updates. You can update small areas of a complex application with less risk of disrupting the entire system.

Additionally, each microservice can scale independently. Instead of scaling the entire application as a single unit, you scale out specific microservices that require more processing power or network bandwidth. such fine-grained scalability provides for greater control of your system and helps to reduce overall costs as you scale portions of your system, not everything.
...
Microservices go hand in hand with software containerization. Developers package the code, dependencies, and runtime for a service as a single unit, called an [image](https://docs.docker.com/glossary/?term=image). Images are stored in a [container registry](https://azure.microsoft.com/services/container-registry/). Registries can reside on your laptop, in your data center, or in the public cloud. When needed, you *pull* the image and transform it into a running container instance. It can run on any computer (development laptop, QA, or production) that has the Docker software engine.

### Benefits of containers

Containers provide portability and guarantee consistency across environments. By encapsulating everything into a single package, you *isolate* the application and its dependencies from the underlying environment. If the container runs on a development laptop, it will run in test and in production as well. Containerized workloads dramatically reduce the expense (and risk) of  pre-configuring each environment and then troubleshoot environment-specific issues across them.

Each [container](https://docs.docker.com/glossary/?term=container) runs on a container host, most often [Docker](https://opensource.com/resources/what-docker)). By sharing the underlying operating system, containers have a much smaller footprint than full virtual machine (VM). The smaller size increases the number of services that can run on a given host.
...
However, modern microservice systems are often large and don't perform well on a local development machine. Some development teams containerize their development cycle. They write code in Docker containers and run the application in a shared development cluster in the cloud. 
...
In the previous figure, Developer Susie Walker and team share a development instance of their entire application in an [Azure Kubernetes Service](https://azure.microsoft.com/services/kubernetes-service/) cluster (in blue). Using Azure Dev Spaces, Susie creates a personal development space with a private copy of the Hotel Service. She then works with her copy of the hotel service, but seamlessly runs it against the entire application in the AKS development cluster. Suzie doesn't need to install the full code base on her local computer or have to mock or replicate dependencies. Until Susie deploys her updated copy of the Hotel Service to the dev cluster, other team members run against the older version in the cluster. We discuss this upcoming product, Azure Dev Spaces, later in this book.
...
Cloud-native systems depend upon many different ancillary resources such as data stores, message brokers, monitoring, and identity services. The ancillary services are known as [backing services](https://12factor.net/backing-services). Figure 1-7 shows many common backing services required by cloud-native systems.

![Cloud-native backing services](media/cloud-native-backing-services.png)

**Figure 1-7**. Cloud-native backing services

Cloud-native applications are best built using *managed backing services* from a cloud provider. The provider manages these resources at scale and bears the responsibility for security, scalability, and maintenance. Your services simply consume each backing service via a RESTFul endpoint.
...
Chances are you’re not in the message broker business. So, why incur the cost of owning this service when someone else can do it better and for less? No matter how skilled you may be, it's unlikely that you can match the reliability and performance of [Azure Service Bus](https://azure.microsoft.com/services/service-bus/), a fully managed message broker service in Azure. The team working on Azure Service Bus specializes in message broker technology - that’s their business. While you, instead, sell consumer items, provide medical services, or deliver packages. Do what you're good at and let others help you with the rest. Focus your effort on building outstanding customer functionality, not becoming experts in open-source backing services.

Finally, managed backing services are supported by the cloud provider. Open a ticket and the provider fixes it. You don't have to scourge the web trying to make sense of error messages from your open-source backing services.
...
A microservices architecture moves your system from in-process to network communication. In a distributed environment, what will you do when Service B isn't responding to a call from Service A? What will you do when Service C is temporarily unavailable, but other services calling it, back up, and degrade the system performance?
...
Cloud-native systems consist of many microservices. Manually integrating, testing, and deploying each would be labor intentive.
...
If you had a small handful of microservices, your DevOps team *might* be able to manually manage the workload. They would need to pay close attention to system performance and traffic patterns to manage and scale the services correctly. However, with 10, 100 or even 1,000 microservices, all bets are off.
