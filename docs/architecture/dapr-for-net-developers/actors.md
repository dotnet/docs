...
The `Program` file is also the place to register the specific actor types. The example below, registers the `ScoreActor` using the `AddActors` extension method:
