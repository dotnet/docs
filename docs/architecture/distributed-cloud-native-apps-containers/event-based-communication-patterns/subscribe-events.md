...
In microservices-based architectures, you should choose availability and tolerance, and you should de-emphasize strong consistency. Therefore, in most modern microservice-based applications, you usually do not want to use distributed transactions in messaging, as you do when you implement [distributed transactions](https://learn.microsoft.com/previous-versions/windows/desktop/ms681205(v=vs.85)) based on the Windows Distributed Transaction Coordinator (DTC) with [MSMQ](/previous-versions/windows/desktop/legacy/ms711472(v=vs.85)).
...
- Using the full [Event Sourcing pattern](https://learn.microsoft.com/azure/architecture/patterns/event-sourcing).
...
Figure 7-22 shows the architecture for the first of these approaches.
...
**Figure 7-22**. Atomicity when publishing events to the event bus

The approach illustrated in Figure 7-22 is missing an additional worker microservice that is in charge of checking and confirming the success of the published integration events. In case of failure, that additional checker worker microservice can read events from the table and republish them, that is, repeat step number 2.

About the second approach: you use the EventLog table as a queue and always use a worker microservice to publish the messages. In that case, the process is like that shown in Figure 7-23. This shows an additional microservice, and the table is the single source when publishing events.

![Diagram of atomicity when publishing with a worker microservice.](./media/atomicity-publish-worker-microservice.png)
...
The event handler needs to verify whether the product exists in any of the basket instances. It also updates the item price for each related basket line item. Finally, it creates an alert to be displayed to the user about the price change, as shown in Figure 7-24.

![Screenshot of a browser showing the price change notification on the user cart.](media/display-item-price-change.png)

**Figure 7-24**. Displaying an item price change in a basket, as communicated by integration events
