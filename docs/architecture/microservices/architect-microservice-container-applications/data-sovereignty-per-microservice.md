...
However, data access becomes much more complex when you move to a microservices architecture. But even when ACID transactions can or should be used within a microservice or Bounded Context, the data owned by each microservice is private to that microservice and can only be accessed via its microservice API. The API can be implemented as a REST API, gRPC, SOAP or anything else, but the key point is that the API encapsulates or hides the implementation details.
