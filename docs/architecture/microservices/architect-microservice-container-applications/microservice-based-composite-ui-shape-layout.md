...
In composite UI application, each UI section is generated by a UI composition microservice, that acts like a mini-gateway. Each of those UI composition microservices would be similar to a small API Gateway. But in this case each one is responsible for a small UI area.
