# .NET Core SDK


* [Overview](sdk-overview.md)
* [.NET Core Command Line Tools](cli/index.md)
    * [Overview](cli/overview.md)
    * [.NET Core CLI extensibility model](cli/extensibility.md)
    * [Using .NET Core CLI tools in Continuous Integration](cli/using-ci-with-cli.md)
    * [dotnet](cli/dotnet.md)
    * [dotnet-new](cli/dotnet-new.md)
    * [dotnet-restore](cli/dotnet-restore.md)
    * [dotnet-run](cli/dotnet-run.md)
    * [dotnet-build](cli/dotnet-build.md)
    * [dotnet-test](cli/dotnet-test.md)
    * [dotnet-pack](cli/dotnet-pack.md)
    * [dotnet-publish](cli/dotnet-publish.md)
    * [dotnet-install-script](cli/dotnet-install-script.md)


