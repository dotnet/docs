...
2. Use the [`dotnet package add`](/dotnet/core/tool/dotnet-package-add) command to add a reference to the Microsoft.XmlSerializer.Generator package.

   ```dotnetcli
   dotnet package add Microsoft.XmlSerializer.Generator -v 8.0.0
   ```

   After running the [`dotnet package add`](/dotnet/core/tool/dotnet-package-add) command, the following lines are added to your *MyApp.csproj* project file:
