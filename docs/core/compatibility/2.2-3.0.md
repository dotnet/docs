...
Migrate your app to use the `Microsoft.AspNetCore.App` framework. Components that were previously available in `Microsoft.AspNetCore.All` are **still available on NuGet**. Those components will now be deployed with your app instead of being included in the shared framework.
...
In ASP.NET Core 1.1, we introduced the `Microsoft.AspNetCore.Mvc.Razor.ViewCompilation` (MVC precompilation tool) package that added support for publish time compilation of Razor files (*.cshtml* files). Starting in ASP.NET Core 2.1, we introduced the [Razor SDK](/aspnet/core/razor-pages/sdk?view=aspnetcore-2.1) that expanded on the features of the precompilation tool by adding support for build and publish time of Razor files. The SDK verifies the correctness of *.cshtml* files at build time while improving on app startup time. The Razor SDK is on by default and no gesture is required to start using it.

In ASP.NET Core 3.0, the earlier MVC precompilation tool was removed. Earlier versions of the package will continue receiving important bug and security fixes as part of our patch release. 

For users of ASP.NET Core 2.1 or later, migrate to the Razor SDK if you haven't already. If you're using the precompilation tool and are unable to migrate to the Razor SDK because of bugs or missing features, file an issue at https://github.com/aspnet/Razor/issues.
...
As part of addressing https://github.com/aspnet/AspNetCore/issues/4849, ASP.NET Core MVC will trim the suffix `Async` from action names by default. This change affects routing and link generation.
...
As of ASP.NET Core 3.0 Preview 6, there's a new `RequireAuthenticatedSignIn` flag on `AuthenticationOptions` that is `true` by default. Set this flag to `false` to restore the old behavior.
...
SignalR's `HubConnectionContext` constructors are changing to accept an options type, rather than multiple parameters, to future-proof adding options. This change replaces two constructors with a single constructor that accepts an options type.
...
As part of [aspnet/AspNetCore#9759](https://github.com/aspnet/AspNetCore/pull/9759), a new `GetFallbackPolicyAsync` method was added to `IAuthorizationPolicyProvider`. This fallback policy will be used by the authorization middleware when no policy is specified.
