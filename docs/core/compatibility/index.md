...
The following sections describes the categories of changes made to .NET Core APIs and their impact on application compatibility.
...
- **Changing the return type of a member to an unrelated type or derived type.**

   For example, a method that returns an <xref:System.Object> cannot be modified to return a <xref:System.String>. It is possible, however, to change the return type from a derived type to a less derived type or to a base type. For example, changing the return type of a method from a <xref:System.String> to an <xref:System.Object> is not a breaking change.
