...
The <xref:System.Runtime.Loader.AssemblyLoadContext> class was introduced for .NET Core and is not available in .NET Framework. This article attempts to supplement the <xref:System.Runtime.Loader.AssemblyLoadContext> API documentation with conceptual information.
