...
> [!NOTE]
> In some Linux environments (for example under *systemd*) the default extraction will not work because `$HOME` is not defined. In such cases it's recommended to set `$DOTNET_BUNDLE_EXTRACT_BASE_DIR` explicitly (for *systemd* a good alternative seems to be the `%h` variable).
