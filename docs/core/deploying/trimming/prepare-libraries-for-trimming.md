...
Set `<IsTrimmable>true</IsTrimmable>` (in .NET 6+) in a `<PropertyGroup>` tag your library project file. This will mark your assembly as "trimmable" and enable trim warnings for that project. Being "trimmable" means when your library is considered compatible with trimming and should have no trim warnings whne building the library. When used in a trimmed application, the assembly will have its unused members trimmed in the final output.
