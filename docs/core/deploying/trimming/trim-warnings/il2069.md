...
Value stored in field 'field' does not satisfy 'DynamicallyAccessedMembersAttribute' requirements 'requirements'. The parameter 'parameter' of method 'method' does not have matching annotations. The source value must declare at least the same requirements as those declared on the target location it is assigned to.
