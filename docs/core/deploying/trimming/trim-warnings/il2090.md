...
'this' argument does not satisfy satisfy in call to 'to'. The generic parameter 'parameter' of 'of' does not have matching annotations. The source value must declare at least the same requirements as those declared on the target location it is assigned to.
