...
- `PerfCollect` requires additional capabilities in order to run the `perf` tool.  The minimal set of capabilities required are [`PERFMON`](https://man7.org/linux/man-pages/man7/capabilities.7.html) and [`SYS_PTRACE`](https://man7.org/linux/man-pages/man7/capabilities.7.html).  Some environments require [`SYS_ADMIN`](https://man7.org/linux/man-pages/man7/capabilities.7.html).  Be sure the container is [started with the necessary capabilities](https://docs.docker.com/engine/reference/run/#runtime-privilege-and-linux-capabilities).  If the minimal set does not work, then try with the full set.
