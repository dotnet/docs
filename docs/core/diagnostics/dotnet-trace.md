...
- **`-- <command>` (for target applications running .NET 5.0 only)**
  ...
  After the collection configuration parameters, the user can append `--` followed by a command to start a .NET application with at least a 5.0 runtime. This may be helpful when diagnosing issues that happen early in the process, such as startup performance issue or assembly loader and binder errors. Note that the target application being traced must be running .NET 5.0 or later to be able to use this feature.
