...
|`COMPlus_EnableCrashReport` or `DOTNET_EnableCrashReport`|(Requires .NET 6 or greater) If set to 1, the runtime generates a json formatted crash report which includes information about the threads and stack frames of the crashing application. The crash report name is the dump path/name with .crashreport.json appended.
|`COMPlus_CreateDumpVerboseDiagnostics` or `DOTNET_CreateDumpVerboseDiagnostics`|(Requires .NET 7 or greater) If set to 1, enables verbose diagnostic logging of the dump process.|0|
|`COMPlus_CreateDumpLogToFile` or `DOTNET_CreateDumpLogToFile`|(Requires .NET 7 or greater) If set, it is the path of the file to write the diagnostic messages. If unset the diagnostic messages will be written to the console of the crashing application.|
