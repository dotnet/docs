...
- <xref:System.Diagnostics.Trace?displayProperty=nameWithType>
  - Only enabled when `TRACE` is defined via adding `#define TRACE` to your source or specifying the option `/d:TRACE` when compiling.
...
- <xref:System.Diagnostics.Debug?displayProperty=nameWithType>
  - Only enabled when `DEBUG` is defined via adding `#define DEBUG` to your source or specifying the option `/d:DEBUG` when compiling.
