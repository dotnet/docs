---
title: Diagnose Managed Memory Leak with dotnet-gcdump
description: In this article, you'll learn how to diagnose a managed memory leak using the global tool dotnet-gcdump.
ms.date: 12/13/2022
---

# Debug Managed Memory Leak in .NET Core using dotnet-gcdump

**This article applies to: ✔️** .NET Core 3.1 SDK and later versions

In this tutorial, you'll learn how to debug a managed memory leak in .NET Core using dotnet-gcdump. Using the provided example [ASP.NET Core web app](/samples/dotnet/samples/diagnostic-scenarios) source code repository, you can cause a managed memory leak intentionally. This can happen when the Garbage Collector is unable to access a growing amount of memory, as a result of the memory continuously being referenced. You'll learn how you can use various tools to diagnose this scenario with several key pieces of diagnostics data.
...
> - Determine managed memory usage with [dotnet-counters](dotnet-counters.md)
> - Use [dotnet-trace](dotnet-trace.md) for trace generation
> - Profile performance in PerfView
> - Diagnose and solve managed memory leak
...
- [dotnet-counters](dotnet-counters.md) to monitor managed memory usage.

## Managed Memory Leak

Before attempting to collect diagnostics data, you need to observe a possibility of a managed-memory leak. Run the [sample application](/samples/dotnet/samples/diagnostic-scenarios) using the following command from the project root directory.
...
Take note of the process ID from your command output. Our process ID was `22884`, but yours will be different. To check the current amount of memory leaked, use the [dotnet-counters](dotnet-counters.md) tool command:

```dotnetcli
dotnet-counters monitor --refresh-interval 1 -p 22884
```

The `refresh-interval` is the number of seconds between the counter polling Heap values. The output should be similar to the following:
...
With the web app running, immediately after startup, no managed memory is being linked as can be seen by the value of '0' on LOH size. Navigate to the `api/diagscenario/memleak` route with `60000` as the route parameter:

`http://localhost:5000/api/diagscenario/memleak/{60000}`
