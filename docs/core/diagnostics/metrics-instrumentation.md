...
- The ObservableCounter and ObservableGauge instrument types don't guarantee thread safety. The callbacks occur on a thread that's not usually synchronized with the code that updates the values. When the callback functions are called by multiple threads, it's your responsibility to guarantee the delegates' thread safety.
