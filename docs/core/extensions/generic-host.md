...
The Worker Service templates create a .NET Generic Host, <xref:Microsoft.Extensions.Hosting.HostBuilder>. The Generic Host can be used with other types of .NET applications, such as Console apps. A Generic Host is not specific to Web applications, for Web apps use the <xref:Microsoft.AspNetCore.Hosting.WebHostBuilder?displayProperty=nameWithType>. For more information on the web host, see [ASP.NET Core Web Host](/aspnet/core/fundamentals/host/web-host).
...
The preceding code is generated by the .NET templates.
