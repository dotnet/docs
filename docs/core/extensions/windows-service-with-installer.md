...
For the app to call a separate process, install the [CliWrap](https://www.nuget.org/packages/CliWrap) NuGet package as a convenience. To install the `CliWrap` package, use the following command. (If you're using an SDK version of .NET 9 or earlier, use the `dotnet add package` form instead.)
