---
author: adegeo
ms.author: adegeo
ms.date: 03/04/2026
ms.topic: include
---

1. Open your .NET project or solution in your development environment.
1. Start the `modernize-dotnet` agent using one of these methods:

   - **Visual Studio**: Right-click the solution or project in **Solution Explorer** and select **Modernize**. Or open the **GitHub Copilot Chat** window and type `@modernize-dotnet`.
   - **Visual Studio Code**: Open the **GitHub Copilot Chat** panel and type `@modernize-dotnet`.
   - **GitHub Copilot CLI**: Type `@modernize-dotnet` followed by your upgrade or migration request.
   - **GitHub.com**: Use the `modernize-dotnet` coding agent in your repository.

1. Tell the `modernize-dotnet` agent what to upgrade or migrate.
