...
This article discusses porting library code to .NET Core so that it can run cross-platform. This topic discusses the .NET Standard specification, technologies that aren't available on .NET Core, how to account for the smaller number of APIs available on .NET Core, how to use the tooling that ships with .NET Core SDK, and the recommended approaches to porting your code.

## Prerequisites

This article assumes you're using Visual Studio 2017 or later. .NET Core isn't supported on previous versions of Visual Studio.

This article also assumes that you understand the [recommended porting process](index.md) and that you've resolved any issues with [third-party dependencies](third-party-deps.md).

## Targeting .NET Standard

The best way to build a cross-platform library for .NET Core is to target [.NET Standard](~/docs/standard/library.md). .NET Standard is the formal specification of .NET APIs that are intended to be available on all .NET runtimes. It's supported by the .NET Core runtime.

What this means is that you'll have to make a tradeoff between APIs you can use and platforms you can support and pick the version of the .NET Standard that best suits the tradeoff you wish to make.

Currently, there are seven versions to consider: .NET Standard 1.0 through 1.6. If you pick a higher version, you get access to more APIs at the cost of running on fewer targets. If you pick a lower version, your code can run on more targets but at the cost of fewer APIs available to you.

For your convenience, here is a matrix of each .NET Standard version and each specific area it runs on:

[!INCLUDE [net-standard-table](~/includes/net-standard-table.md)]

A key concept to understand is that **a project targeting a lower version of the .NET Standard cannot reference a project targeting a higher version**. For example, a project targeting the .NET Standard version 1.2 cannot reference projects that target .NET Standard version 1.3 or higher. Projects **can** reference lower versions, so a project targeting .NET Standard 1.3 can reference a project targeting .NET Standard 1.2 or lower.

We recommend that you pick the lowest possible .NET Standard version that permits your class library to work on the targets where you need it to work and use that throughout your project. Learn more in the [.NET Standard](../../standard/library.md) topic.

## Key technologies not yet available on the .NET Standard or .NET Core

You may be using some technologies available for the .NET Framework that are not currently available for .NET Core. Each of the following sub-sections corresponds to one of those technologies. Alternative options are listed if it's feasible for you to adopt them.

### App Domains

AppDomains can be used for different purposes on the .NET Framework. For code isolation, we recommend separate processes and/or containers as an alternative. For dynamic loading of assemblies, we recommend the new <xref:System.Runtime.Loader.AssemblyLoadContext> class.
...
* <xref:System.Runtime.Serialization.DataContractSerializer> for both XML and JSON
* <xref:System.Xml.Serialization.XmlSerializer> for XML
...
1. A retargeted PCL project which supports .NET Standard.
...
1. Select ".NET Core" under Visual C#.
1. Select the "Class Library (.NET Core)" template. 
...
1. Under Build, select "Convert to .NET Standard".
...
1. If ApiPort was ran, take a quick glance at the report.
1. Copy all of your code over into a new .NET Core project.
1. Work out compiler errors until it compiles, referring to the portability report if needed.
1. Repeat as needed.
...
1. Address issues by using different APIs which are portable.
1. Keep note of any areas where you can't use a direct alternative.
1. Repeat steps 1-3 for all projects you're porting until you're confident each is ready to be copied over into a .NET Core project.
1. Copy the code into a new .NET Core projects.
1. Work out any issues that you've kept note of.
...
1. Run ApiPort on a project.
1. Understand where in your code each non-portable type is being used and how that affects overall portability.
   ...
   a. Is it easy to isolate code that isn't portable so you can deal with it more easily?
   a. Would you need to refactor your code?
   a. For those types which aren't portable, are there alternative APIs that accomplish the same task? For example, if you're using the `WebClient` class, you may be able to use the `HttpClient` class instead.
   a. Are there different portable APIs you can use to accomplish a task, even if it's not a drop-in replacement? For example, if you're using `XmlSchema` to help parse XML, but you don't require XML schema discovery, you could use `System.Linq.Xml` APIs and hand-parse the data.
...
1. Make any changes needed to get the code to compile. Much of this may require adding NuGet package dependencies to your `project.json` file.
1. Run tests and make any needed adjustments.
1. Pick the next layer of code to port over and repeat steps 2 and 3!
