---
title: .NET Core Command Line Tools RC3 Architecture | Microsoft Docs
description: RC3 brings about certain changes to the way the overall .NET Core tools are layered. 
...
# High-level overview of changes in CLI RC3
...
![RC3 tools high-level architecture](media/p3-arch.png)
...
These commands still do what you expect them to do (new up a project, build it, publish it, pack it and so on). Majority of the options are not changed, and are still there, and you can consult either the commands' help screens (using `dotent <command> --help`) or RC3 documentation on this site to get familiar with any changes. 
...
## Conclusion 
This document outlined at a high-level the changes that are happening to the overall CLI tooling architecture and functioning that are coming with RC3. It has introduced the notion of the shared SDK component as well as explained how the CLI commands function, from a technical perspective, in RC3.