...
| 10.0.300         | 'Restore' multi-targeting with duplicate target frameworks | Enables [multi-targeting with duplicate target frameworks](/nuget/consume-packages/package-references-in-project-files#multi-targeting-with-duplicate-frameworks) using `TargetFrameworkAlias`. Emits [NU1018](/nuget/reference/errors-and-warnings/nu1018) error when duplicate target frameworks are present but not all target frameworks have a `TargetFrameworkAlias` set. |
| 10.0.300         | 'Restore' `TargetFrameworkAlias` character validation | Emits [NU1019](/nuget/reference/errors-and-warnings/nu1019) error when a `TargetFrameworkAlias` value contains path separator characters (`/` or `\`). Emits [NU1019](/nuget/reference/errors-and-warnings/nu1019) warning when a `TargetFrameworkAlias` value contains non-ASCII characters. |
| 11.0.100         | 'Restore' MonoAndroid framework deprecation | Emits [NU1703](/nuget/reference/errors-and-warnings/nu1703) warning when a package uses the deprecated `MonoAndroid` framework instead of a modern .NET target framework moniker. |
| 11.0.100         | 'Restore' `TargetFrameworkAlias` non-ASCII character validation | Promotes [NU1019](/nuget/reference/errors-and-warnings/nu1019) from a warning to an error when a `TargetFrameworkAlias` value contains non-ASCII characters. |
