...
The `IsPublishable` property allows the `Publish` target to run. This property only affects processes that use *.csproj* files and the `Publish` target, such as the [dotnet publish](../tools/dotnet-publish.md) command. Visual Studio is not affected because Visual Studio uses the `PublishOnly` target. The default value is `true`.
