...
| `--report-azdo-groups` | 2.3.0 | Enables or disables per-assembly log groups. Valid values are `on` and `off`. Starting with MTP 2.4.0, the default is `off`; earlier versions default to `on`. Requires `--report-azdo`. |
| `--report-azdo-annotations` | 2.3.0 | Enables or disables annotations for failed and skipped tests. Valid values are `on` (default) and `off`. Requires `--report-azdo`. |
...
> [!NOTE]
> The **MTP version** column lists the MTP release in which each option first became available. MTP 2.4.0 is unreleased as of August 2026.
