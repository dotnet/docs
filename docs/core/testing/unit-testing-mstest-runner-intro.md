...
The MSTest runner supports the [runsettings](unit-testing-platform-extensions-vstest-bridge.md#runsettings-support) through the command-line option `--settings`. The full list of supported MSTest entries is available [here](./unit-testing-mstest-configure.md#runsettings). The following commands show examples.
