...
> [!NOTE]
> You only need `DoNotParallelize` when you've enabled in-assembly parallelization. When parallelization is disabled (the default), `DoNotParallelize` has no effect.
