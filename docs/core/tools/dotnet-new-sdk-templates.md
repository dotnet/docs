...
- **`-f|--framework <FRAMEWORK>`**
  ...
  The ability to create a project for an earlier TFM depends on having that version of the SDK installed. For example, if you have only the .NET SDK 7.0.x installed, then the only value available for `--framework` is `net7.0`. If you install the .NET SDK 6.0.x, the value `net6.0` becomes available for `--framework`. If you install the .NET Core 3.1 SDK, `netcoreapp3.1` becomes available, and so on. So by specifying `--framework netcoreapp3.1` you can target .NET Core 3.1 even while running `dotnet new` in the .NET SDK 6.0.x.
