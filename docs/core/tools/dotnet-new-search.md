...
- **`TEMPLATE_NAME`**
  ...
  > [!NOTE]
  > Starting .NET SDK 6.0.100, it is possible to use `<TEMPLATE_NAME>` argument also after `--search` option: `dotnet new --search web`. The command provides same result as `dotnet new web --search`.
