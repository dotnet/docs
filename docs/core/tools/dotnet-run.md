...
- **`--project <PATH>`**
  ...
  Specifies the path of the project file to run (folder name or full path). If not specified, it defaults to the current directory. The `-p` abbreviation for `--project` is deprecated starting in .NET 6 Preview SDK. For more information, see [`-p` option for `dotnet run` is deprecated](../compatibility/sdk/6.0/deprecate-p-option-dotnet-run.md).
