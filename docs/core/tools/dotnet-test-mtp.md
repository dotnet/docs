...
description: The dotnet test command with Microsoft Testing Platform is used to execute unit tests in a given project.
ms.date: 12/02/2024
ai-usage: ai-assisted
---
# dotnet test with Microsoft Testing Platform

**This article applies to:** ✔️ .NET 10 SDK and later versions

> [!NOTE]
> Microsoft Testing Platform is a modern, lightweight test platform that offers faster test execution, stricter defaults, and better determinism, but requires opt-in configuration. This article covers `dotnet test` with Microsoft Testing Platform. For information about using `dotnet test` with the classic VSTest platform (the default), see [dotnet test with VSTest](dotnet-test-vstest.md).

## Name

`dotnet test` - Executes unit tests for the given scope using Microsoft Testing Platform.
...
Microsoft Testing Platform is an opt-in alternative to VSTest that provides faster test execution, stricter defaults, and better determinism. To enable Microsoft Testing Platform, you need to specify the test runner in the [`global.json`](global-json.md) file:
...
> [!WARNING]
> When Microsoft.Testing.Platform is opted in via `global.json`, `dotnet test` expects all test projects to use Microsoft.Testing.Platform. It's an error if any of the test projects use VSTest.
