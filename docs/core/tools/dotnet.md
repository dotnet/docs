...
- **`--depsfile`**
  ...
  Path to the *deps.json* file. A *deps.json* file is a configuration file that contains information about dependencies necessary to run the application. This file is generated by .NET Core SDK.
...
- **`--roll-forward-on-no-candidate-fx <N>`** **Available in .NET Core 2.x SDK.**
  ...
  *Note: Starting with .NET Core 3.0 this option is superseded by `--roll-forward` and that option should be used instead.
...
- `DOTNET_ROLL_FORWARD_TO_PRERELEASE` **Available starting with .NET Core 3.x.**
  ...
  If set to `1` (enabled) enables rolling forward to pre-release version from a release version. By default (`0` - disabled) when a release version of .NET Core runtime is requested, roll forward will only consider installed release versions.
...
- `DOTNET_ROLL_FORWARD_ON_NO_CANDIDATE_FX` **Available in .NET Core 2.x.**
  ...
  This setting is superseded in .NET Core 3.0 with `DOTNET_ROLL_FORWARD`. The new settings should be used instead.
...
- `COREHOST_TRACE`, `COREHOST_TRACEFILE`, `COREHOST_TRACE_VERBOSITY`
  ...
  * `COREHOST_TRACE=[0/1]` - default is `0` - tracing disabled. If set to `1` the diagnostics tracing is enabled.
  * `COREHOST_TRACEFILE=<file path>` **Available starting with .NET Core 3.x.** - only has effect if tracing is enabled via `COREHOST_TRACE=1`. When set the tracing information is written to the specified file, otherwise the tracing information is written to `stderr`.
  * `COREHOST_TRACE_VERBOSITY=[1/2/3/4]` **Available starting with .NET Core 3.x.** - default is `4`. The setting is used only when tracing is enabled via `COREHOST_TRACE=1`.
  ...
  Typical way to get a detailed trace information about the application startup is to set `COREHOST_TRACE=1` and `COREHOST_TRACEFILE=host_trace.txt` and then run the application. A new file `host_trace.txt` will be created in the current directory with the detailed information.
