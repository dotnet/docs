...
- **`--runtimeconfig <PATH>`**
  ...
  Path to a *runtimeconfig.json* file. The file contains run-time settings and is typically named *\<applicationname>.runtimeconfig.json*. For more information, see [.NET runtime configuration settings](../runtime-config/index.md#runtimeconfigjson).

  This option requires the `exec` subcommand (specify `dotnet exec`).
