...
.NET Core tools are [framework-dependent applications](../deploying/index.md#runtime-dependent), which means they rely on a .NET Core runtime installed on your machine. If the expected runtime isn't found, they follow normal .NET Core runtime roll-forward rules such as:
