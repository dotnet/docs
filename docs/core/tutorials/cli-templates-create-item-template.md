...
The `sourceName` item is what is replaced when the user creates a new item from the template. In this case, the user will run `dotnet new stringext -n test` and the `sourceName` will be replaced with the name the user sets on item creation. The `sourceName` must match the class file and the namespace. In this case test will replace `ExampleTemplate.StringExtensions` in the c# file and the namespace.
