...
```json
{
  ...
  "shortName": "stringext",
  "sourceName":"ExampleTemplate.StringExtensions",
  "tags": {
    "language": "C#",
    "type": "item"
  }
...
The `sourceName` item is what is replaced when the user uses the template. The value of `sourceName` in the config file, is searched for in any file name and file content. In this case test will replace `consoleasync` in the name of the class file. By default, this value uses the name of the current folder. When the `-n` or `--name` parameter is passed with the `dotnet new` command, the value provided is used instead of the current folder.
