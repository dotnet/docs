...
The `dotnet` CLI chooses the SDK version for commands such as `dotnet new` and `dotnet run`. By default, the latest installed SDK is used, including preview versions. You can use a newer SDK than the .NET Core version you're targetting. For example, you may use the .NET Core SDK v2.1.301 when it's installed, even if the project you are working with targets the .NET Core Runtime 2.0. You can take advantage of the latest SDK features and improvements while targeting earlier .NET Core runtime versions. You can target multiple runtime versions of .NET Core on different projects, using the same SDK tools for all projects.
