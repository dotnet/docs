...
There's a lot of moving pieces where asynchronous operations are concerned.  If you're curious about what's happening underneath the covers of `Task` and `Task<T>`, checkout the [Async in-depth](../fundamentals/async-in-depth.md) article for more information.
...
If the work you have is **I/O-bound**, use `async` and `await` *without* `Task.Run`.  You *should not* use the Task Parallel Library.  The reason for this is outlined in the [Async in Depth article](../fundamentals/async-in-depth.md).
...
* [Async in-depth](../fundamentals/async-in-depth.md) provides more information about how Tasks work.
