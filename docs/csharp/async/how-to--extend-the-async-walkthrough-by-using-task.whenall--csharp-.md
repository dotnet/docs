...
You can improve the performance of the async solution in [Walkthrough: Accessing the Web by Using async and await (C#)](../async/walkthrough--accessing-the-web-by-using-async-and-await--csharp-.md) by using the <xref:System.Threading.Tasks.Task.WhenAll*?displayProperty=fullName> method. This method asynchronously awaits multiple asynchronous operations, which are represented as a collection of tasks.  
...
> [!IMPORTANT]
>  The following procedures describe extensions to the async applications that are developed in [Walkthrough: Accessing the Web by Using async and await (C#)](../async/walkthrough--accessing-the-web-by-using-async-and-await--csharp-.md). You can develop the applications by either completing the walkthrough or downloading the code from [Developer Code Samples](http://go.microsoft.com/fwlink/?LinkId=255191).  
>   
...
1.  Add the `ProcessURLAsync` method to the first application that's developed in [Walkthrough: Accessing the Web by Using async and await (C#)](../async/walkthrough--accessing-the-web-by-using-async-and-await--csharp-.md).  
...
1.  Add the following version of `ProcessURLAsync` to the second application that's developed in [Walkthrough: Accessing the Web by Using async and await (C#)](../async/walkthrough--accessing-the-web-by-using-async-and-await--csharp-.md).  
...
-   For either solution, choose the F5 key to run the program, and then choose the **Start** button. The output should resemble the output from the async solutions in [Walkthrough: Accessing the Web by Using async and await (C#)](../async/walkthrough--accessing-the-web-by-using-async-and-await--csharp-.md). However, notice that the websites appear in a different order each time.  
