...
This example demonstrates how to create multicast delegates. A useful property of [delegate](../keywords/delegate--csharp-reference-.md) objects is that multiple objects can be assigned to one delegate instance by using the `+` operator. The multicast delegate contains a list of the assigned delegates. When the multicast delegate is called, it invokes the delegates in the list, in order. Only delegates of the same type can be combined.  
  
 The `-` operator can be used to remove a component delegate from a multicast delegate.  
  
## Example  
 [!code[csProgGuideDelegates#11](../delegates/codesnippet/CSharp/how-to--combine-delegates--multicast-delegates--csharp-programming-guide-_1.cs)]  
