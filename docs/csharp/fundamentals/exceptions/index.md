...
- Code in a `finally` block is executed even if an exception is not thrown. Use a `finally` block to release resources, for example to close any streams or files that were opened in the `try` block.
