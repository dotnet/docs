...
1. Run the program in Debug mode by selecting the the "HelloWorld" button with the green arrow on the toolbar, pressing F5, or selecting **Debug**, **Start Debugging**.
...
1. Let's try to change the value of the variables to see how this affects our program. If the **Immediate window** is not visible, display it by selecting **Debug**, **Windows**, **Immediate**. The **Immediate window** lets you interact with the application you're debugging.

1. You can interactively change the values of variables. Type the following into the immediate window:
...
1. Continue program execution by selecting the **Continue** button in the toolbar, or by selecting **Debug**, **Continue**. The resulting console window should resemble the following image. Note that the values displayed in the console window also correspond to the changes we made in the **Immediate window**.
...
1. Right-click on the red dot that represents the breakpoint. From the pop-up menu, select **Conditions...**. Visual Studio opens the **Breakpoint Settings** dialog shown in the following figure.
...
1. Select the **Locals** window, which shows the values of variables that are local to the method that is currently executing, in this case the `Main` method.

1. Note that the value of the `name` variable is "", or [String.Empty](xref:System.String.Empty). We can confirm this by entering the following statement in the **Immediate window**:
...
1. Clear the breakpoint by clicking on the dot in the left margin of the code window.
...
1. Select **Debug**, **Step Into** from the Visual Studio menu, or press the F11 key. Visual Studio highlights and displays an arrow beside the line of that will be executed next, as the following figure shows.
...
1. Select **Debug**, **Step Into**, or press the F11 key. Visual Studio now highlights the next line to be execute. As the figure shows, it has taken 3 milliseconds to execute the code between the last statement and this one. `args` remains the only declared variable, and the console window is still blank.
...
1. Select **Debug**, **Step Into**, or press the F11 key. The **Autos** window now shows the value of the `date` variable after the assignment from the [DateTime.Now](xref:System.DateTime.Now) property. The console window is unchanged.
...
1. Select **Debug**, **Step Out**, or press Shift and the F11 key. This stops step-by-step execution. The console window displays a message and waits for us to press a key.
...
When you press F5 or select **Build Solution** from the **Build** menu, Visual Studio compiles the release version of your console application for you to test. You can then run and test it as you did the debug verison of the application.
