---
title: How to index (C# Guide)
description: A collection of quick tips and 
author: billwagner
ms.author: wiwagn
ms.date: 08/16/2017
ms.topic: article
ms.prod: .net
ms.devlang: devlang-csharp
---

# How to index

The How to section of the C# Guide is where you can find quick answers
to common questions and scenarios. The articles in this section are
linked in multiple ways in both the table of contents and the index so you
can quickly scan to find the answers you need.

## strings

These address the most common scenarios for working with strings.




Split

Searching

concatenate strings.



