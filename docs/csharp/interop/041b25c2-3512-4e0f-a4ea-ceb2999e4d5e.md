...
2.  Add the following `using` directives to the top of the code file.  
     ...
     [!code[csProgGuideOfficeHowTo#1](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_1.cs)]  
  
### To create a list of bank accounts  
  
1.  Paste the following class definition into **Program.cs**, under the `Program` class.  
  
     [!code[csProgGuideOfficeHowTo#2](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_2.cs)]  
  
2.  Add the following code to the `Main` method to create a `bankAccounts` list that contains two accounts.  
  
     [!code[csProgGuideOfficeHowTo#3](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_3.cs)]  
...
1.  Add the following method to the `Program` class to set up an Excel worksheet.  
     ...
     Method [Add](http://go.microsoft.com/fwlink/?LinkId=210910) has an optional parameter for specifying a particular template. Optional parameters, new in [!INCLUDE[csharp_dev10_long](../classes-and-structs/includes/csharp_dev10_long_md.md)], enable you to omit the argument for that parameter if you want to use the parameter's default value. Because no argument is sent in the following code, `Add` uses the default template and creates a new workbook. The equivalent statement in earlier versions of C# requires a placeholder argument: `ExcelApp.Workbooks.Add(Type.Missing)`.  
  
     [!code[csProgGuideOfficeHowTo#4](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_4.cs)]  
...
2.  Add the following code at the end of `DisplayInExcel`. The code inserts values into the first two columns of the first row of the worksheet.  
     ...
     [!code[csProgGuideOfficeHowTo#5](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_5.cs)]  
...
3.  Add the following code at the end of `DisplayInExcel`. The `foreach` loop puts the information from the list of accounts into the first two columns of successive rows of the worksheet.  
     ...
     [!code[csProgGuideOfficeHowTo#7](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_6.cs)]  
  
4.  Add the following code at the end of `DisplayInExcel` to adjust the column widths to fit the content.  
  
     [!code[csProgGuideOfficeHowTo#13](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_7.cs)]  
     ...
     [!code[csProgGuideOfficeHowTo#14](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_8.cs)]  
  
     [!INCLUDE[csharp_dev10_long](../classes-and-structs/includes/csharp_dev10_long_md.md)], and later versions, converts the returned `Object` to `dynamic` automatically if the assembly is referenced by the [/link](../compiler-options/-link--csharp-compiler-options-.md) compiler option or, equivalently, if the Excel **Embed Interop Types** property is set to true. True is the default value for this property.  
  
### To run the project  
  
1.  Add the following line at the end of `Main`.  
  
     [!code[csProgGuideOfficeHowTo#8](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_9.cs)]  
...
1.  To illustrate additional ways in which [!INCLUDE[csharp_dev10_long](../classes-and-structs/includes/csharp_dev10_long_md.md)], and later versions, enhances Office programming, the following code opens a Word application and creates an icon that links to the Excel worksheet.  
  
     Paste method `CreateIconInWordDoc`, provided later in this step, into the `Program` class. `CreateIconInWordDoc` uses named and optional arguments to reduce the complexity of the method calls to [Add](http://go.microsoft.com/fwlink/?LinkId=210937) and [PasteSpecial](http://go.microsoft.com/fwlink/?LinkId=147099). These calls incorporate two other new features introduced in [!INCLUDE[csharp_dev10_long](../classes-and-structs/includes/csharp_dev10_long_md.md)] that simplify calls to COM methods that have reference parameters. First, you can send arguments to the reference parameters as if they were value parameters. That is, you can send values directly, without creating a variable for each reference parameter. The compiler generates temporary variables to hold the argument values, and discards the variables when you return from the call. Second, you can omit the `ref` keyword in the argument list.  
  
     The `Add` method has four reference parameters, all of which are optional. In [!INCLUDE[csharp_dev10_long](../classes-and-structs/includes/csharp_dev10_long_md.md)], or later versions, you can omit arguments for any or all of the parameters if you want to use their default values. In [!INCLUDE[csharp_orcas_long](../interop/includes/csharp_orcas_long_md.md)] and earlier versions, an argument must be provided for each parameter, and the argument must be a variable because the parameters are reference parameters.  
  
     The `PasteSpecial` method inserts the contents of the Clipboard. The method has seven reference parameters, all of which are optional. The following code specifies arguments for two of them: `Link`, to create a link to the source of the Clipboard contents, and `DisplayAsIcon`, to display the link as an icon. In [!INCLUDE[csharp_dev10_long](../classes-and-structs/includes/csharp_dev10_long_md.md)], you can use named arguments for those two and omit the others. Although these are reference parameters, you do not have to use the `ref` keyword, or to create variables to send in as arguments. You can send the values directly. In [!INCLUDE[csharp_orcas_long](../interop/includes/csharp_orcas_long_md.md)] and earlier versions, you must send a variable argument for each reference parameter.  
  
     [!code[csProgGuideOfficeHowTo#9](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_10.cs)]  
  
     In [!INCLUDE[csharp_orcas_long](../interop/includes/csharp_orcas_long_md.md)] or earlier versions of the language, the following more complex code is required.  
  
     [!code[csProgGuideOfficeHowTo#10](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_11.cs)]  
  
2.  Add the following statement at the end of `Main`.  
  
     [!code[csProgGuideOfficeHowTo#11](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_12.cs)]  
  
3.  Add the following statement at the end of `DisplayInExcel`. The `Copy` method adds the worksheet to the Clipboard.  
  
     [!code[csProgGuideOfficeHowTo#12](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_13.cs)]  
...
4.  Find **Embed Interop Types** in the list of properties, and change its value to **False**. Equivalently, you can compile by using the [/reference](../compiler-options/-reference--csharp-compiler-options-.md) compiler option instead of [/link](../compiler-options/-link--csharp-compiler-options-.md) at a command prompt.  
...
1.  Replace the two calls to `AutoFit` in `DisplayInExcel` with the following statement.  
     ...
     [!code[csProgGuideOfficeHowTo#15](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_14.cs)]  
     ...
     [!code[csProgGuideOfficeHowTo#16](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_15.cs)]  
...
3.  Compare the statement in step 1 with the following code, which shows the arguments that are required in [!INCLUDE[csharp_orcas_long](../interop/includes/csharp_orcas_long_md.md)] or earlier versions.  
  
     [!code[csProgGuideOfficeHowTo#17](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_16.cs)]  
  
## Example  
 The following code shows the complete example.  
  
 [!code[csProgGuideOfficeHowTo#18](../interop/codesnippet/CSharp/041b25c2-3512-4e0f-a4ea-ceb2999e4d5e_17.cs)]  
