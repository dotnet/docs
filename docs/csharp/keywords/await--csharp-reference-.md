...
The `await` operator is applied to a task in an asynchronous method to suspend the execution of the method until the awaited task completes. The task represents ongoing work.  
 ...
 The asynchronous method in which `await` is used must be modified by the [async](../keywords/async--csharp-reference-.md) keyword. Such a method, defined by using the `async` modifier, and usually containing one or more `await` expressions, is referred to as an *async method*.  
...
## Exceptions  
 ...
 For examples of error handling in async methods, see [try-catch](../keywords/try-catch--csharp-reference-.md).  
