...
- Change an async entry point's return type to <xref:System.Threading.Tasks.Task> or <xref:System.Threading.Tasks.Task%601> (**CS4009**). Starting with C# 7.1, the [`Main` method](../../fundamentals/program-structure/main-command-line.md#main-return-values) can be `async`, but it must return `Task` or `Task<int>` - `async void` and `async int` aren't valid entry point signatures.
