...
| [select](select-clause.md)   | Specifies the type and shape that the elements in the returned sequence have when the query executes. |
| [group](group-clause.md)     | Groups query results according to a specified key value. |
| [into](into.md)              | Provides an identifier that serves as a reference to the results of a join, group, or select clause. |
...
| [let](let-clause.md)         | Introduces a range variable to store subexpression results in a query expression. |
