...
There is a predefined implicit conversion from [byte](../builtin-types/integral-numeric-types.md), [ushort](../builtin-types/integral-numeric-types.md), or [char](char.md) to `uint`. Otherwise you must use a cast. For example, the following assignment statement will produce a compilation error without a cast:
