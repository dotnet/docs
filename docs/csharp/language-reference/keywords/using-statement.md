...
## Remarks  
 ...
 Often, when you use an `IDisposable` object, you should declare and instantiate it in a `using` statement. The `using` statement calls the <xref:System.IDisposable.Dispose%2A> method on the object in the correct way, and (when you use it as shown earlier) it also causes the object itself to go out of scope as soon as <xref:System.IDisposable.Dispose%2A> is called. Within the `using` block, the object is read-only and cannot be modified or reassigned.
 
 Note however some `IDisposable` objects are designed for re-use in which case it may be inefficent to create and dispose a new instance within a method call which `using` necessitates. For example an instance <xref:System.Net.Http.HttpClient> can be re-used multiple times without creating and disposing an instance every time. In such cases take care to correctly dispose the instance since `using` cannot be used.
