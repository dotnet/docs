...
The conditional operator `?:`, commonly known as the ternary conditional operator, evaluates one of two expressions and returns its result depending on the value of a Boolean expression. Beginning with C# 7.2, the [conditional ref expression](#conditional-ref-expression) returns the reference to the result of one of the two expressions.
...
The `condition` expression must evaluate to `true` or `false`. If `condition` evaluates to `true`, the `consequence` expression is evaluated and its result becomes the result of the operation. If `condition` evaluates to `false`, the `alternative` expression is evaluated and its result becomes the result of the operation. A conditional expression evaluates only one of the two expressions: either `consequence` or `alternative`.
...
Beginning with C# 7.2, you can use the conditional ref expression to return the reference to the result of one of the two expressions. You can assign that reference to a [local ref](../keywords/ref.md#ref-locals) or [local ref readonly](../keywords/ref.md#ref-readonly-locals) variable, use as a [reference return value](../keywords/ref.md#reference-return-values) or as a [`ref` method parameter](../keywords/ref.md#passing-an-argument-by-reference).
...
In case of the conditional ref expression, the type of `consequence` and `alternative` must be the same.
