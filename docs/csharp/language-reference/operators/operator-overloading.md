...
- A unary operator has one input parameter; a binary operator has two input parameters. In each case at least one parameter must be of type `T` or `T?`, where `T` is the type that contains the operator declaration.
- It returns either a `boolean` value (comparison operators) or an instance of the type defining the operator (all other operators).
