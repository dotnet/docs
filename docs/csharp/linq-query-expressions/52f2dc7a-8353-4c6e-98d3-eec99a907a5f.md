...
2.  Copy the `StudentClass` class from [How to: Query a Collection of Objects](../linq-query-expressions/how-to--query-a-collection-of-objects--csharp-programming-guide-.md) and paste it into namespace `PredicateFilters` underneath class `Program`. `StudentClass` provides a list of `Student` objects.  
  
3.  Comment out the `Main` method in `StudentClass`.  
  
4.  Replace class `Program` with the following code.  
  
     [!code[csProgGuideLINQ#26](../arrays/codesnippet/CSharp/52f2dc7a-8353-4c6e-98d3-eec99a907a5f_1.cs)]  
...
2.  Copy the following method and paste it into class `DynamicPredicates`.  
     ...
     [!code[csProgGuideLINQ#27](../arrays/codesnippet/CSharp/52f2dc7a-8353-4c6e-98d3-eec99a907a5f_2.cs)]  
