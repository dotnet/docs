...
## Example  
 The following code populates an object graph with the `Address`, `PurchaseOrder`, and `PurchaseOrderItem` classes from the [Sample XML File: Typical Purchase Order (LINQ to XML)](../linq/sample-xml-file--typical-purchase-order--linq-to-xml-1.md) XML document.  
