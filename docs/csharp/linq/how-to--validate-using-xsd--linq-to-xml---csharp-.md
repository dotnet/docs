...
## Example  
 The following example validates that the XML document from [Sample XML File: Customers and Orders (LINQ to XML)](../linq/sample-xml-file--customers-and-orders--linq-to-xml-2.md) is valid per the schema from [Sample XSD File: Customers and Orders](../linq/sample-xsd-file--customers-and-orders1.md). It then modifies the source XML document. It changes the `CustomerID` attribute on the first customer. After the change, orders will then refer to a customer that does not exist, so the XML document will no longer validate.  
