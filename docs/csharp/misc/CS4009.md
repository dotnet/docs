...
> [!IMPORTANT]
> Starting with C# 7.1, the `Main` method can have an `async` modifier. For more information, see the [Async main](../whats-new/csharp-version-history.md#c-version-70) section of the C# Version history article. For information about how to select the C# language version, see the [Select the C# language version](../language-reference/configure-language-version.md) article.
