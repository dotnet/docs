...
The event 'event' can only appear on the left hand side of += or -= (except when used from within the type 'type')  
 ...
 Outside of the class it is defined in, an [event](../language-reference/keywords/event.md) can only add or subtract references. For more information, see [Events](../events-overview.md).  
