...
Before C# 11, interface members could not combine `static` with `override`, `virtual`, or `abstract`.
In C# 11, interface methods, properties, events, and operators can combine `static` with `abstract` or `virtual` by using `static abstract` or `static virtual`.
