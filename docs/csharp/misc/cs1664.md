---
description: "Compiler Error CS1664"
title: "Compiler Error CS1664"
ms.date: 07/20/2015
f1_keywords: 
  - "CS1664"
helpviewer_keywords: 
  - "CS1664"
ms.assetid: 3515024e-5da3-4a69-baac-96e0ede9bc32
---
# Compiler Error CS1664

Fixed size buffer of length 'length' and type 'type' is too big  
  
 The maximum size of a fixed-size buffer (as determined by the length multiplied by the element size) is 2^31 = 2147483648.
