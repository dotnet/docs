---
description: "Learn more about: Compiler Error CS2011"
title: "Compiler Error CS2011"
ms.date: 07/20/2015
f1_keywords: 
  - "CS2011"
helpviewer_keywords: 
  - "CS2011"
ms.assetid: 82d71d7e-7ce0-428a-b9fe-ce8e81a07abd
---
# Compiler Error CS2011

Unable to open response file 'file'  
  
 A response file was specified in a compilation, but the compiler was unable to locate and open the file.  
  
 For more on response files, see [**ResponseFiles** (Specify Response File)](../language-reference/compiler-options/miscellaneous.md#responsefiles).
