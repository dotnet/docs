...
'member': CLS-compliant interfaces must have only CLS-compliant members  
 ...
 In an assembly marked with `[assembly:CLCSompliant(true)]`, an interface contains a member marked with `[CLCSompliant(false)]`. Remove one of the Common Language Specification (CLS) compliance attributes. For more information about CLS Compliance, see [\<PAVE OVER> Writing CLS-Compliant Code](https://msdn.microsoft.com/library/bhc3fa7f.aspx) and [Language Independence and Language-Independent Components](https://msdn.microsoft.com/library/12a7a7h3).  
