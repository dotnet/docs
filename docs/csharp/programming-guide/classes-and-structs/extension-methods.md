...
You invoke the extension method in your code with instance method syntax. The intermediate language (IL) generated by the compiler translates your code into a call to the static method. The principle of encapsulation isn't really being violated. Extension members can't access private variables in the type they're extending.
...
<< TODO: rewrite for static extensions >>
In general, you'll probably be calling extension members far more often than implementing your own. Because extension members are called by using instance syntax, no special knowledge is required to use them from client code. To enable extension members for a particular type, just add a `using` directive for the namespace in which the methods are defined. For example, to use the standard query operators, add this `using` directive to your code:
