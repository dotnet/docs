...
> [!NOTE]
> Empty finalizers should not be used. When a class contains a finalizer, an entry is created in the `Finalize` queue. This queue is processed when the garbage collector calls the finalizer. An empty finalizer just causes a needless loss of performance.
