...
> [!NOTE]
> The `Main` method returns `Task`, despite not having a `return` expression&mdash;this is by design. For more information, see [Evaluation of a void-returning async functions](/dotnet/csharp/language-reference/language-specification/classes#14153-evaluation-of-a-void-returning-async-function).
...
Now, the thread working on the breakfast isn't blocked while awaiting any started task that hasn't yet been finished. For some applications, this change is all that's needed. A GUI application still responds to the user with just this change. However, for this scenario, you want more. You don't want each of the component tasks to be executed sequentially. It's better to start each of the component tasks before awaiting the previous task's completion.
...
The asynchronously prepared breakfast took roughly 20 minutes, this time savings is because some tasks run concurrently.
...
The series of `await` statements at the end of the preceding code can be improved by using methods of the `Task` class. One of those APIs is <xref:System.Threading.Tasks.Task.WhenAll%2A>, which returns a <xref:System.Threading.Tasks.Task> that completes when all the tasks in its argument list have been completed, as shown in the following code:
