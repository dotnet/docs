...
|SingleOrDefault|Returns the only element of a collection, or the only element that satisfies a condition. Returns a default value if a collection is empty, or no such element exists. Throws an <xref:System.InvalidOperationException> if a collection contains more than one element, or more than one element satisfy a condition.|Not applicable.|<xref:System.Linq.Enumerable.SingleOrDefault%2A?displayProperty=nameWithType><br /><br /> <xref:System.Linq.Queryable.SingleOrDefault%2A?displayProperty=nameWithType>|  
