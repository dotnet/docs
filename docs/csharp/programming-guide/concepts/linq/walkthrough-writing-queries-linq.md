...
1. As described in [Query Syntax and Method Syntax in LINQ](/dotnet/csharp/linq/get-started/write-linq-queries), some query operations can only be expressed by using method syntax. The following code calculates the total score for each `Student` in the source sequence, and then calls the `Average()` method on the results of that query to calculate the average score of the class.
