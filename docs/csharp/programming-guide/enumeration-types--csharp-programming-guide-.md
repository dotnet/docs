...
An enumeration type (also named an enumeration or an enum) provides an efficient way to define a set of named integral constants that may be assigned to a variable. For example, assume that you have to define a variable whose value will represent a day of the week. There are only seven meaningful values which that variable will ever store. To define those values, you can use an enumeration type, which is declared by using the [enum](../keywords/enum--csharp-reference-.md) keyword.  
  
 [!code[csProgGuideEnums#1](../programming-guide/codesnippet/CSharp/enumeration-types--csharp-programming-guide-_1.cs)]  
  
 By default the underlying type of each element in the enum is [int](../keywords/int--csharp-reference-.md). You can specify another integral numeric type by using a colon, as shown in the previous example. For a full list of possible types, see [enum (C# Reference)](../keywords/enum--csharp-reference-.md).  
...
-   In [!INCLUDE[vsprvs](../classes-and-structs/includes/vsprvs_md.md)], IntelliSense lists the defined values.  
 ...
 [!code[csProgGuideEnums#4](../programming-guide/codesnippet/CSharp/enumeration-types--csharp-programming-guide-_2.cs)]  
...
>  It is possible to assign any arbitrary integer value to `meetingDay`. For example, this line of code does not produce an error: `meetingDay = (Days) 42`. However, you should not do this because the implicit expectation is that an enum variable will only hold one of the values defined by the enum. To assign an arbitrary value to a variable of an enumeration type is to introduce a high risk for errors.  
 ...
 [!code[csProgGuideEnums#3](../programming-guide/codesnippet/CSharp/enumeration-types--csharp-programming-guide-_3.cs)]  
...
## Enumeration Types as Bit Flags  
 ...
 [!code[csProgGuideEnums#2](../programming-guide/codesnippet/CSharp/enumeration-types--csharp-programming-guide-_4.cs)]  
  
 To set a flag on an enum, use the bitwise `OR` operator as shown in the following example:  
  
 [!code[csProgGuideEnums#6](../programming-guide/codesnippet/CSharp/enumeration-types--csharp-programming-guide-_5.cs)]  
  
 To determine whether a specific flag is set, use a bitwise `AND` operation, as shown in the following example:  
  
 [!code[csProgGuideEnums#7](../programming-guide/codesnippet/CSharp/enumeration-types--csharp-programming-guide-_6.cs)]  
...
## Using the System.Enum Methods to Discover and Manipulate Enum Values  
 ...
 [!code[csProgGuideEnums#5](../programming-guide/codesnippet/CSharp/enumeration-types--csharp-programming-guide-_7.cs)]  
  
 For more information, see <xref:System.Enum?displayProperty=fullName>.  
  
 You can also create a new method for an enum by using an extension method. For more information, see [How to: Create a New Method for an Enumeration](../classes-and-structs/how-to--create-a-new-method-for-an-enumeration--csharp-programming-guide-.md).  
