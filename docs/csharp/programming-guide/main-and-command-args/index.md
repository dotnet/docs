...
The addition of `async` and `Task`, `Task<int>` returns types simplifies program code when console applications need to start and `await` asynchronous operations in `Main`.
