...
The addition of `async` and `Task`, `Task<int>` returns types is only avaialble with C# 7.1 and higher. This feature simplifies program code when console applications need to start and `await` asyncnhronous operations in `Main`.
