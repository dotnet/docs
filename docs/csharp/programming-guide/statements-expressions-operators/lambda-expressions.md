...
A *lambda expression* is an expression of any of the two following forms:

- [Expression lambda](#expression-lambdas) that have an expression as its body:

  ```csharp
  (input-parameters) => expression
  ```

- [Statement lambda](#statement-lambdas) that have a statement block as its body:
