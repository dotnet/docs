...
You can convert a [string](../../../csharp/language-reference/keywords/string.md) to a number by using methods in the <xref:System.Convert> class or by using the `TryParse` method found on the various numeric types (int, long, float, etc.).  
 ...
 If you have a string, it is slightly more efficient and straightforward to call a <xref:System.Int32.TryParse%2A> method (for example, `int.TryParse("11", out var number);`).  Using a <xref:System.Convert> method is more useful for general objects that implement <xref:System.IConvertible>.  
