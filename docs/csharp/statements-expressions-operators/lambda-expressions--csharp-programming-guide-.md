...
```  
 ...
 The `=>` operator has the same precedence as assignment (`=`) and is [right associative](../statements-expressions-operators/operators--csharp-programming-guide-.md) (see “Associativity” section of the Operators article).  
  
 Lambdas are used in method-based [!INCLUDE[vbteclinq](../classes-and-structs/includes/vbteclinq_md.md)] queries as arguments to standard query operator methods such as <xref:System.Linq.Enumerable.Where*>.  
  
 When you use method-based syntax to call the <xref:System.Linq.Enumerable.Where*> method in the <xref:System.Linq.Enumerable> class (as you do in [!INCLUDE[vbteclinq](../classes-and-structs/includes/vbteclinq_md.md)] to Objects and [!INCLUDE[sqltecxlinq](../linq/includes/sqltecxlinq_md.md)]) the parameter is a delegate type <xref:System.Func`2?displayProperty=fullName>. A lambda expression is the most convenient way to create that delegate. When you call the same method in, for example, the <xref:System.Linq.Queryable?displayProperty=fullName> class (as you do in [!INCLUDE[vbtecdlinq](../keywords/includes/vbtecdlinq_md.md)]) then the parameter type is an <xref:System.Linq.Expressions.Expression?displayProperty=fullName><Func\> where Func is any of the Func delegates with up to sixteen input parameters. Again, a lambda expression is just a very concise way to construct that expression tree. The lambdas allow the `Where` calls to look similar although in fact the type of object created from the lambda is different.  
 ...
 Lambdas are not allowed on the left side of the [is](../keywords/is--csharp-reference-.md) or [as](../keywords/as--csharp-reference-.md) operator.  
  
 All restrictions that apply to anonymous methods also apply to lambda expressions. For more information, see [Anonymous Methods](../statements-expressions-operators/anonymous-methods--csharp-programming-guide-.md).  
