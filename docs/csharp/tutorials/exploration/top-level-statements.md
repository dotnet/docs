...
Top-level statements can only be in one file, and that file can also contain namespaces and types at the end of the file, after the top-level statements. But for this tutorial you put the animation method in a separatte file to make it more readily reusable.
