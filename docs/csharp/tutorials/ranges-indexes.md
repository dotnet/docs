...
The addition of indexes and ranges provide clear, concise syntax to access a single element or a sub-range of elements in a sequence. An index expression typically returns the type of the elements of a collection. A range expression typically returns the same sequence type as the source sequence.

If a type provides an [indexer](../programming-guide/indexers/index.md) with an <xref:System.Index> or <xref:System.Range> parameter, it explicitly supports indices or ranges respectively. When the type provides an indexer that takes a single <xref:System.Range> parameter, it may choose to return a different sequence type, such as <xref:System.Memory.Span%601?displayProperty=nameWithType>.
