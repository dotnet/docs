...
A type is **countable** if it has a property named `Length` or `Count` with an accessible getter and a return type of `int`. A countable type that doesn't explicitly support indices or ranges may provide an implicit support for them. For more information, see the [Implicit Index support](~/_csharpstandard/standard/ranges.md#implicit-index-support) and [Implicit Range support](~/_csharpstandard/standard/ranges.md#implicit-range-support) sections of the [feature proposal note](~/_csharpstandard/standard/ranges.md). Ranges using implicit range support return the same sequence type as the source sequence.
