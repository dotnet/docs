...
In your `Extensions.cs` file, type in or copy the method below. This new extension method will create a new file called `debug.log` within your project's directory and record what query is currently being executed to the log file. This extension method can be appended to any query to mark that the query executed.
