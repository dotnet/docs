...
Before we begin, make sure that the following lines are at the top of the `Program.cs` file generated by `dotnet new console`:
...
Now that we have all of the references we need, let's consider what constitutes a deck of cards. Commonly, a deck of playing cards has four suits, and each suit has thirteen values. Normally, you might consider creating a `Card` class right off the bat and populating a collection of `Card` objects by hand. With LINQ, we can be more concise than the usual way of dealing with creating a deck of cards. Instead of creating a `Card` class, let's use two collections that will represent suites and ranks, respectively. We'll create a really simple pair of [*iterator methods*](../iterators.md#enumeration-sources-with-iterator-methods) that will generate the ranks and suits as enumerables of strings:
...
Now, let's use these iterator methods to create our deck of cards. We'll place the LINQ query in our `Main` method. Let's take a look at it:
...
```
Note that the above still uses `SelectMany` and that what is returned by the method syntax query is exactly the same as what is returned by the query syntax version. Choose which syntax works best for your situation: for instance, if you're working in a team where some of the members have difficulty with method syntax, try to prefer using query syntax.
...
Let's give our extension methods a home: add a new *static* class file to your program called `Extensions.cs`, and then start building out our first extension method: 
...
namespace LinqFaroShuffle
{
    public static class Extensions
    {
        ...
        {
            // Our implementation will go here soon enough
        }
    }
}
```
...
Naturally, since we split the deck into halves, we'll need to join those halves together. In code, this means we'll be enumerating both of the sequences we acquired through `Take()` and `Skip()` at once, *interleaving* the elements, and creating one object: our now-shuffled deck of cards. Writing a LINQ method that works with two sequences requires that you understand how `IEnumerable` works.
...
Run the code we've got so far. Notice how the deck rearranges on each shuffle; after 8 shuffles (iterations of the do-while loop), the deck returns to its original configuration it was in when we first created it from our starting LINQ query.

## Optimizations

The sample you've built so far executes an *outer shuffle*, where the top and bottom cards stay the same on each run. Let's make one change: we'll use an *inner shuffle* instead, where all 52 cards change position. For an inner shuffle, you interleave the deck so that the first card in the bottom half becomes the first card in the deck. That means the last card in the top half becomes the bottom card. This is a simple change to a singular line of code. Update our current shuffle query by switching the positions of `Take()` and `Skip()`. This will change the order of the top and bottom halves of the deck:
...
There are a number of reasons for this. Let's tackle one of the major causes: inefficient use of [*lazy evaluation*](../programming-guide/concepts/linq/deferred-execution-and-lazy-evaluation-in-linq-to-xml.md).
