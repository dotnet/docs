...
Next, let's write some test code. You can make use of C#'s [pattern matching](/dotnet/csharp/fundamentals/functiona/pattern-matching) feature to determine a light's capabilities by examining which interfaces it supports.  The following method exercises the supported capabilities of each light:
