...
The first step to migrating a .NET Framework app to .NET Core is to update it to use `<PackageReference>` NuGet references. Visual Studio makes this simple. Just select the project's *packages.config* file in Visual Studio's **Solution Explorer**, and then right-click **Migrate packages.config to PackageReference**.
