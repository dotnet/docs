...
* The result of any arithmetic operation performed on two date and time values whose `DateTime.Kind` properties both equal [DateTimeKind.Utc](http://dotnet.github.io/api/System.DateTimeKind.html#System_DateTimeKind_Utc) reflects the actual time interval between the two values. Similarly, the comparison of two such date and time values accurately reflects the relationship between times.

* The result of any arithmetic or comparison operation performed on two date and time values whose `DateTime.Kind` properties both equal [DateTimeKind.Local](http://dotnet.github.io/api/System.DateTimeKind.html#System_DateTimeKind_Local) or on two date and time values with different `DateTime.Kind` property values reflects the difference in clock time between the two values. 
...
In this example, the [DateTimeOffset.CompareTo](http://dotnet.github.io/api/System.DateTimeOffset.html#System_DateTimeOffset_CompareTo_System_DateTimeOffset_) method indicates that the current local time and the current UTC time are equal, and subtraction of `DateTimeOffset` values indicates that the difference between the two times is [TimeSpan.Zero](http://dotnet.github.io/api/System.TimeSpan.html#System_TimeSpan_Zero). 
...
The [System.TimeZoneInfo](http://dotnet.github.io/api/System.TimeZoneInfo.html) class does not provide any methods that automatically apply adjustment rules when you perform date and time arithmetic. However, you can do this by converting the time in a time zone to UTC, performing the arithmetic operation, and then converting from UTC back to the time in the time zone. For details, see [How to: Use Time Zones in Date and Time Arithmetic](essentials\datetime\UseTimeZonesinArithmetic.md).
...
[How to: Use Time Zones in Date and Time Arithmetic](essentials\datetime\UseTimeZonesinArithmetic.md)
