...
If you want users to be able to programmatically distinguish between some error conditions, you can create your own user-defined exceptions. .NET Core provides a hierarchy of exception classes ultimately derived from the base class [Exception](http://dotnet.github.io/api/System.Exception.html). Each of these classes defines a specific exception, so in many cases you only have to catch the exception. You can also create your own exception classes by deriving from the **Exception** class.
