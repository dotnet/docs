...
1. Assembly A is created with a strong name using one of the following methods:
    ...
    - Creating a cryptographic key pair using the [Strong Name tool (Sn.exe)](../tools/sn-exe-strong-name-tool.md) and assigning that key pair to the assembly using either a command-line compiler or the [Assembly Linker (Al.exe)](../tools/al-exe-assembly-linker.md). The Windows SDK provides both Sn.exe and Al.exe.
...
|[Sn.exe (Strong Name Tool)](../tools/sn-exe-strong-name-tool.md)|Describes the tool included in the .NET Framework that helps create assemblies with strong names. This tool provides options for key management, signature generation, and signature verification.|
|[Al.exe (Assembly Linker)](../tools/al-exe-assembly-linker.md)|Describes the tool included in the .NET Framework that generates a file that has an assembly manifest from modules or resource files.|
