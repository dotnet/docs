...
If the **\<supportedRuntime>** element with the `sku` attribute is present in the configration file and the installed .NET Framework version is lower then the specified supported version, the application will fail to run and instead display the message asking to install the supported version. Otherwise, the application will attempt to run on any installed version, and can possibly crash or behave unexpectedly if it is not compatible with that version. Therefore, it is recommended to include this element in the application configuration file for the easier error diagnostics (the configuration file automatically generated by Visual Studio when creating new project already contains it).
