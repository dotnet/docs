...
|certificateValidationMode|Optional enumeration. Specifies one of the modes used to validate credentials. This attribute is of the <!-- TODO: review code entity reference <xref:System.Servicemodel.Security.X509CertificateValidationMode>  --> type. If set to `Custom`, then a `customCertificateValidator` must also be supplied. The default is `ChainTrust`.|  
