...
> [!NOTE]
> The <xref:System.Data.DataTable.CaseSensitive> property of the `DataTable` does not affect this behavior. The `CaseSensitive` property applies to the data in a table and affects sorting, searching, filtering, enforcing constraints, and so on, but not to references to the columns, relations, and constraints.
