...
> [!NOTE]
>  The `DataSet` and `DataTable` objects inherit from <xref:System.ComponentModel.MarshalByValueComponent>and support the <xref:System.Runtime.Serialization.ISerializable> interface for remoting. These are the only ADO.NET objects that can be remoted. For more information, see [Remote Objects](https://msdn.microsoft.com/library/72x4h507(v=vs.100).aspx).  
...
## See Also  
 ...
 [Validating Data](https://msdn.microsoft.com/library/t3b36awf.aspx)   
