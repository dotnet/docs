...
> [!NOTE]
> As part of the predicate in a `Where…Contains` clause, you can use an <xref:System.Array>, a <xref:System.Collections.Generic.List`1>, or a collection of any type that implements the <xref:System.Collections.Generic.IEnumerable`1> interface. You can also declare and initialize a collection within a LINQ to Entities query. See the next example for more information.
