...
#### Using a Data Store Connection  
 When the data store connection is available, call <xref:System.Data.Common.DbProviderServices.GetProviderManifestToken?displayProperty=nameWithType> to return the token that is passed to the <xref:System.Data.Common.GetProviderManifest%2A> method, which returns <xref:System.Data.Common.DbProviderManifest>. This method delegates to the provider's implementation of `GetDbProviderManifestToken`.  
