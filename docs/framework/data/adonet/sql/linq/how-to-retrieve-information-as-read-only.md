...
When you do not intend to change the data, you can increase the performance of queries by seeking read-only results.
 ...
 You implement read-only processing by setting <xref:System.Data.Linq.DataContext.ObjectTrackingEnabled*> to `false`.

> [!NOTE]
> When <xref:System.Data.Linq.DataContext.ObjectTrackingEnabled*> is set to `false`, <xref:System.Data.Linq.DataContext.DeferredLoadingEnabled*> is implicitly set to `false`.
