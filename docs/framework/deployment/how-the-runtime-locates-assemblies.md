...
4. [Probes for the assembly](#step4) using the following steps:
    ...
    1. If configuration and publisher policy do not affect the original reference and if the bind request was created using the <xref:System.Reflection.Assembly.LoadFrom*?displayProperty=nameWithType> method, the runtime checks for location hints.
...
The following code provides a simple example of an application configuration file. This example adds a <xref:System.Diagnostics.TextWriterTraceListener> to the <xref:System.Diagnostics.Debug.Listeners*> collection to enable recording debug information to a file.
