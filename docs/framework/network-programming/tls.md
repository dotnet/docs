...
Since .NET Framework is dependent on [`Schannel`](/windows-server/security/tls/tls-ssl-schannel-ssp-overview) on Windows, the operating system dictates which versions can be negotiated and which version will be used.
...
All of the registry keys have the same effect whether you're doing HTTP networking (<xref:System.Net.ServicePointManager>) or TCP sockets networking (<xref:System.Net.Security.SslStream>).
