...
|**-e**[**xecution**] {**on** &#124; **off**}|Turns on or off the mechanism that checks for the permission to run before code starts to execute. **Note:**  This switch is removed in the .NET Framework 4 and later versions. For more information, see [Security Changes](/previous-versions/dotnet/framework/windows-identity-foundation/security-changes).|
