...
The Resource File Generator (Resgen.exe) converts text (.txt or .restext) files and XML-based resource format (.resx) files to common language runtime binary (.resources) files that can be embedded in a runtime binary executable or satellite assembly. For more information, see [Create resource files](../../core/extensions/create-resource-files.md).
...
> [!NOTE]
> For information about the format of .txt and .restext files, see the "Resources in Text Files" section of [Create resource files](../../core/extensions/create-resource-files.md).
