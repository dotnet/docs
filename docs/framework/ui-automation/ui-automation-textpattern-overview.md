...
Because TextPattern relies on cross-process calls for most of its functionality, it does not provide a caching mechanism to improve performance when processing content. This is unlike other control patterns in Microsoft UI Automation that can be accessed using the <xref:System.Windows.Automation.AutomationElement.GetCachedPattern*> or <xref:System.Windows.Automation.AutomationElement.TryGetCachedPattern*> methods.
