...
1. By modifying the <xref:System.Threading.ExecutionContext> settings to suppress the flow on a per-thread basis (see the <xref:System.Threading.ExecutionContext.SuppressFlow*>, <xref:System.Security.SecurityContext.SuppressFlow*>, and <xref:System.Security.SecurityContext.SuppressFlowWindowsIdentity*> methods).
