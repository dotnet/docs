...
- THREAD_PRIORITY_TIME_CRITICAL
 ...
 The CLR calls `SetPriority` when the value of the <xref:System.Threading.Thread.Priority*?displayProperty=nameWithType> is modified by user code. A host can define its own algorithms for thread priority assignment, and is free to ignore this request.
