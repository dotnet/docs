...
|`STARTUP_ETW`|Specifies that event tracing for Windows (ETW) is enabled for common language runtime events. Beginning with Windows Vista, event tracing is always enabled, so this flag has no effect. See [Controlling .NET Framework Logging](../../performance/controlling-logging.md).|
|`STARTUP_ARM`|Specifies that application domain resource monitoring is enabled. See the <xref:System.AppDomain.MonitoringIsEnabled?displayProperty=nameWithType> property and [`<appDomainResourceMonitoring>` Element](../../configure-apps/file-schema/runtime/appdomainresourcemonitoring-element.md).|
