...
- If you set the variables in an [ICorDebug::CreateProcess](../../../core/unmanaged-api/debugging/icordebug/icordebug-createprocess-method.md) call, they will apply only to the application that you are running at the time. (They will also apply to other applications started by that application that inherit the environment.)
