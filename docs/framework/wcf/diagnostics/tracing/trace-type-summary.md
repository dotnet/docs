...
---
# Trace Type Summary
[Source Levels](/dotnet/api/system.diagnostics.sourcelevels&view=netframework-4.8) defines various trace levels: Critical, Error, Warning, Information, and Verbose, as well as provides description of the `ActivityTracing` flag, which toggles the output of trace boundary and activity transfer events.  
  
 You can also review [TraceEventType](/dotnet/api/system.diagnostics.traceeventtype&view=netframework-4.8) for the types of traces which can be emitted from <xref:System.Diagnostics>.  
