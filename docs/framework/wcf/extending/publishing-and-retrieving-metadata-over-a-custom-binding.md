...
## Publishing Metadata Over a Custom Binding
 ...
 The HTTP GET metadata endpoints and HTTPS GET metadata endpoints are enabled by setting the <xref:System.ServiceModel.Description.ServiceMetadataBehavior.HttpGetEnabled*?displayProperty=nameWithType> or <xref:System.ServiceModel.Description.ServiceMetadataBehavior.HttpsGetEnabled?displayProperty=nameWithType> properties to `true`. The bindings for these endpoints cannot be configured.
