...
To configure a Windows Communication Foundation (WCF) service endpoint to be interoperable with ASP.NET Web service clients, use the <xref:System.ServiceModel.BasicHttpBinding?displayProperty=nameWithType> type as the binding type for your service endpoint.  
 ...
 To make the metadata for a WCF service available to ASP.NET Web service proxy generation tools (that is, [Web Services Description Language Tool (Wsdl.exe)](https://msdn.microsoft.com/library?url=/library/cptools/html/cpgrfwebservicesdescriptionlanguagetoolwsdlexe.asp), [Web Services Discovery Tool (Disco.exe)](https://msdn.microsoft.com/library?url=/library/cptools/html/cpgrfwebservicesdiscoverytooldiscoexe.asp), and the Add Web Reference feature in Visual Studio), you should expose an HTTP/GET metadata endpoint.  
