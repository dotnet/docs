...
2. Set the <xref:System.Net.HttpWebRequest.Method> property of the <xref:System.Net.HttpWebRequest> to `POST` and the <xref:System.Net.HttpWebRequest.ContentType> property to `"text/plain"`. This tells the service that the code is sending data and that data is in plain text.
...
3. Call <xref:System.Net.HttpWebRequest.GetRequestStream*> to get the request stream, create the data to send, write that data to the request stream, and close the stream.
...
4. Get the response from the service by calling <xref:System.Net.HttpWebRequest.GetResponse*> and display the response data to the console.
