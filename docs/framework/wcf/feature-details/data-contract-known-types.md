...
#### Example 4
 ...
 In the following example, a data contract stores a number and an operation to perform on the number. The `Numbers` data member can be an integer, an array of integers, or a <xref:System.Collections.Generic.List`1> that contains integers.
...
- <xref:System.Runtime.Serialization.DataContractSerializer.KnownTypes*>
