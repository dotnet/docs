...
8. Link the two interfaces into a duplex contract by setting the <xref:System.ServiceModel.ServiceContractAttribute.CallbackContract> property in the primary interface to the type of the callback interface.
...
2. Set the variable to the object reference returned by the <xref:System.ServiceModel.OperationContext.GetCallbackChannel*> method of the <xref:System.ServiceModel.OperationContext> class.
