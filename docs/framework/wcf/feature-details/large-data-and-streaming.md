...
-   Are not available in their entirety when the transfer is initiated.  
 ...
 When sending large amounts of data you will need to set the maxAllowedContentLength IIS setting (for more information see [Configuring IIS Request Limits](http://go.microsoft.com/fwlink/?LinkId=253165)) and the maxReceivedMessageSize binding setting (for example [<xref:System.ServiceModel.BasicHttpBinding.MaxReceivedMessageSize](xref:System.ServiceModel.HttpBindingBase.MaxReceivedMessageSize%2A) or <xref:System.ServiceModel.NetTcpBinding.MaxReceivedMessageSize%2A>). The maxAllowedContentLength property defaults to 28.6 M and the maxReceivedMessageSize property defaults to 64KB.  
