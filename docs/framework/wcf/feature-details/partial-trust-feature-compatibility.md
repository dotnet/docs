...
- All serializable `[DataMember]` fields or properties in a `[DataContract]` type must be public and read/write. The serialization and deserialization of [readonly](https://www.microsoft.com/download/details.aspx?id=55979) fields is not supported when running WCF in a partially-trusted application.  
...
## Other Limitations  
 WCF is generally limited to the security considerations imposed upon it by the hosting application. For example, if WCF is hosted in a XAML Browser Application (XBAP), it is subject to XBAP limitations, as described in [Windows Presentation Foundation Partial Trust Security](/dotnet/framework/wpf/wpf-partial-trust-security).  
