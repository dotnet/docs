...
### Slow Stream Attacks  
 ...
 To mitigate these attacks, set the transport time-outs correctly. [!INCLUDE[crdefault](../../../../includes/crabout-md.md)] [Transport Quotas](../../../../docs/framework/wcf/feature-details/transport-quotas.md). Secondly, never use synchronous `Read` or `Write` operations when working with streams in [!INCLUDE[indigo2](../../../../includes/indigo2-md.md)].  
...
-   Use callbacks to ensure that the object is in a valid state. The callback marked with the <xref:System.Runtime.Serialization.OnDeserializedAttribute> attribute is especially useful because it runs after deserialization is complete and has a chance to examine and correct the overall state. [!INCLUDE[crdefault](../../../../includes/crabout-md.md)] [Version-Tolerant Serialization Callbacks](../../../../docs/framework/wcf/feature-details/version-tolerant-serialization-callbacks.md).  
