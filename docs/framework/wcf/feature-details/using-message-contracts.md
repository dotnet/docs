...
## Controlling Header and Body Part Names and Namespaces
 ...
 You can change these defaults by manipulating the <xref:System.ServiceModel.MessageContractMemberAttribute.Name*?displayProperty=nameWithType> and <xref:System.ServiceModel.MessageContractMemberAttribute.Namespace*?displayProperty=nameWithType> (on the parent class of the <xref:System.ServiceModel.MessageHeaderAttribute> and <xref:System.ServiceModel.MessageBodyMemberAttribute> attributes).
...
[!code-vb[MessageHeaderAttribute#3](../../../../samples/snippets/visualbasic/VS_Snippets_CFX/messageheaderattribute/vb/services.vb#3)]
 ...
 To suppress the wrapper element, set the <xref:System.ServiceModel.MessageContractAttribute.IsWrapped> property to `false`. To control the name and the namespace of the wrapper element, use the <xref:System.ServiceModel.MessageContractAttribute.WrapperName*> and <xref:System.ServiceModel.MessageContractAttribute.WrapperNamespace> properties.
...
- `Relay`
 ...
 When sending a message, these attributes are not emitted by default. You can change this in two ways. First, you may statically set the attributes to any desired values by changing the <xref:System.ServiceModel.MessageHeaderAttribute.Actor*?displayProperty=nameWithType>, <xref:System.ServiceModel.MessageHeaderAttribute.MustUnderstand*?displayProperty=nameWithType>, and <xref:System.ServiceModel.MessageHeaderAttribute.Relay?displayProperty=nameWithType> properties, as shown in the following code example. (Note that there is no `Role` property; setting the <xref:System.ServiceModel.MessageHeaderAttribute.Actor> property emits the `Role` attribute if you are using SOAP 1.2).
...
```
 ...
 The second way to control these attributes is dynamically, through code. You can achieve this by wrapping the desired header type in the <xref:System.ServiceModel.MessageHeader`1> type (be sure not to confuse this type with the non-generic version) and by using the type together with the <xref:System.ServiceModel.MessageHeaderAttribute>. Then, you can use properties on the <xref:System.ServiceModel.MessageHeader`1> to set the SOAP attributes, as shown in the following code example.
...
```
 ...
 On the receiving side, reading these SOAP attributes can only be done if the <xref:System.ServiceModel.MessageHeader`1> class is used for the header in the type. Examine the `Actor`, `Relay`, or `MustUnderstand` properties of a header of the <xref:System.ServiceModel.MessageHeader`1> type to discover the attribute settings on the received message.

 When a message is received and then sent back, the SOAP attribute settings only go round-trip for headers of the <xref:System.ServiceModel.MessageHeader`1> type.

## Order of SOAP Body Parts

 In some circumstances, you may need to control the order of the body parts. The order of the body elements is alphabetical by default, but can be controlled by the <xref:System.ServiceModel.MessageBodyMemberAttribute.Order?displayProperty=nameWithType> property. This property has the same semantics as the <xref:System.Runtime.Serialization.DataMemberAttribute.Order?displayProperty=nameWithType> property, except for the behavior in inheritance scenarios (in message contracts, base type body members are not sorted before the derived type body members). For more information, see [Data Member Order](data-member-order.md).

 In the following example, `amount` would normally come first because it is first alphabetically. However, the <xref:System.ServiceModel.MessageBodyMemberAttribute.Order> property puts it into the third position.
...
- The name of the wrapper element for the response message contract must be the same as the name of the operation suffixed by 'Response'. Use the <xref:System.ServiceModel.MessageContractAttribute.WrapperName> property of the message contract for this.
