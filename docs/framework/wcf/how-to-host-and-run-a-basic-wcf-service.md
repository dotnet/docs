...
The next step in creating a WCF application is to host a WCF service in a console application. This procedure consists of the following steps:
...
A complete listing of the code for this task is provided in the example following the procedure.
...
The steps in the sample code are described as follows:
...
- **Step 2**: Host the service. To host the service, create an instance of the <xref:System.ServiceModel.ServiceHost> class. The constructor takes two parameters: the type of the class that implements the service contract and the base address of the service.
...
- **Step 4**: Enable metadata exchange. Clients use metadata exchange to generate proxies for calling the service operations. To enable metadata exchange, create a <xref:System.ServiceModel.Description.ServiceMetadataBehavior> instance, set its <xref:System.ServiceModel.Description.ServiceMetadataBehavior.HttpGetEnabled%2A> property to `true`, and add the behavior to the <!--zz <xref:System.ServiceModel.ServiceHost.Behaviors%2A>  -->`System.ServiceModel.ServiceHost.Behaviors%2A` collection of the <xref:System.ServiceModel.ServiceHost> instance.
