...
## Creating IDuplexSessionChannel  
 ...
 Once the <xref:System.ServiceModel.Channels.Message> is encoded into bytes, it must be transmitted on the wire. This requires a system for defining message boundaries. WSE 3.0 uses a version of [DIME](https://msdn.microsoft.com/Areas/Epx/Content/500.aspx?aspxerrorpath=/msdnmag/issues/02/12/DIME/) as its framing protocol. `WriteData` encapsulates the framing logic to wrap a byte[] into a set of DIME records.  
