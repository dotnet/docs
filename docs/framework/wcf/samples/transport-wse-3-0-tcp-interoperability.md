...
## Creating IDuplexSessionChannel  
 ...
 Once the <xref:System.ServiceModel.Channels.Message> is encoded into bytes, it must be transmitted on the wire. This requires a system for defining message boundaries. WSE 3.0 uses a version of [DIME](https://msdn.microsoft.com/msdnmag/issues/02/12/DIME/) as its framing protocol. `WriteData` encapsulates the framing logic to wrap a byte[] into a set of DIME records.  
...
1. To run this sample, you must have WSE 3.0 and the WSE `TcpSyncStockService` sample installed. You can download [WSE 3.0 from MSDN](https://www.microsoft.com/download/details.aspx?id=14089).  
