...
3. To run the sample in a single- or cross-machine configuration, follow the instructions in [Running the Windows Communication Foundation Samples](../../../../docs/framework/wcf/samples/running-the-samples.md).  
    ...
    > [!NOTE]
    > For cross-machine configuration, enable the Distributed Transaction Coordinator using the instructions below, and use the WsatConfig.exe tool from the Windows SDK to enable WCF Transactions network support. See [Configuring WS-Atomic Transaction Support](/dotnet/framework/wcf/feature-details/configuring-ws-atomic-transaction-support) for information on setting up WsatConfig.exe.  
