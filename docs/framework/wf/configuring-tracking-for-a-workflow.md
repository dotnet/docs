...
> [!NOTE]
>  If you want to view sample code that shows how to create a custom behavior element that adds a custom tracking participant, refer to the [Tracking](../../../docs/framework/wf/samples/tracking.md) samples.  
...
### Configuring tracking using WorkflowInvoker  
 To configure tracking for a workflow executed using <xref:System.Activities.WorkflowInvoker>, add the tracking provider as an extension to a <xref:System.Activities.WorkflowInvoker> instance. The following code example is from the [Custom Tracking](../../../docs/framework/wf/samples/custom-tracking.md) sample.  
...
6.  Right-click the **Analytic** node and select **Refresh.** Tracking records should be visible in the center pane.  
 ...
 ![Event Viewer showing tracking records](../../../docs/framework/wf/media/trackingeventviewer.PNG "TrackingEventViewer")  
