...
1.  Create the instance store database.
    ...
    2.  Navigate to the sample directory (\WF\Basic\Persistence\SqlStoreExtensibility\CS) and run CreateInstanceStore.cmd in the Visual Studio 2010 command prompt.
...
2.  Open Visual Studio 2010 and load the SqlStoreExtensibility.sln solution and build it by pressing CTRL+SHIFT+B.
...
3.  In a Visual Studio 2010 command prompt, run the QueryInstanceStore.cmd script located in the sample directory (\WF\Basic\Persistence\SqlStoreExtensibility).
