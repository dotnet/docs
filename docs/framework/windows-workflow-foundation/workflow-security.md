...
- When using <xref:System.Activities.DurableInstancing.SqlWorkflowInstanceStore>, the instance store must be secured. For more information, see [SQL Server Best Practices](https://docs.microsoft.com/sql/sql-server/&view=sql-server-ver15).

- Sensitive data in the instance store should be encrypted. For more information, see [SQL Security Encryption](https://docs.microsoft.com/previous-versions/sql/sql-server-2008/cc278098(v=sql.100)).
...
- Windows Communication Foundation (WCF) endpoints used in workflows should be secured. For more information, see [WCF Security Overview](https://docs.microsoft.com/dotnet/framework/wcf/feature-details/security-overview).

- Host-level authorization can be implemented by using <xref:System.ServiceModel.ServiceAuthorizationManager>. See [How To: Create a Custom Authorization Manager for a Service](https://docs.microsoft.com/dotnet/framework/wcf/extending/how-to-create-a-custom-authorization-manager-for-a-service) for details.
