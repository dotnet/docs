...
The generic operators are customizable. To customize the comparison functions, override [Equals](https://msdn.microsoft.com/library/bsc2ak47.aspx) to provide your own custom equality comparison, and then implement <xref:System.IComparable>. The <xref:System.IComparable?displayProperty=fullName> interface has a single method, the <xref:System.IComparable.CompareTo*> method.
