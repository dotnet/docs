...
| + others  | | | |
| `Ident`                      | ~1.5%    | Typed Tree | identifiers - a range and a reference to a string |
| `TType_fun`                  | ~1.5%    | Typed Tree | node for function types, especially in imported metadata |
...
2. Nearly all long-lived memory is related to the Typed Tree nodes of the F# compiler data structure, or the related Abstract IL nodes for .NET assemblies being imported.
