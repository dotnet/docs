...
This type must be public, and you must mark it with the [TypeProvider](https://msdn.microsoft.com/visualfsharpdocs/conceptual/compilerservices.typeproviderattribute-class-%5bfsharp%5d) attribute so that the compiler will recognize the type provider when a separate F# project references the assembly that contains the type. The *config* parameter is optional, and, if present, contains contextual configuration information for the type provider instance that the F# compiler creates.

Next, you implement the [ITypeProvider](https://msdn.microsoft.com/visualfsharpdocs/conceptual/compilerservices.itypeprovider-interface-%5bfsharp%5d) interface. In this case, you use the `TypeProviderForNamespaces` type from the `ProvidedTypes` API as a base type. This helper type can provide a finite collection of eagerly provided namespaces, each of which directly contains a finite number of fixed, eagerly provided types. In this context, the provider *eagerly* generates types even if they aren't needed or used.
...
- This provided type is erased.  Because you indicate that the base type is `obj`, instances will appear as values of type [obj](https://msdn.microsoft.com/visualfsharpdocs/conceptual/core.obj-type-abbreviation-%5bfsharp%5d) in compiled code.
<br />
...
An instance of the provided type will be created with underlying data "The object data". The quoted code includes a conversion to [obj](https://msdn.microsoft.com/visualfsharpdocs/conceptual/core.obj-type-abbreviation-%5bfsharp%5d) because that type is the erasure of this provided type (as you specified when you declared the provided type).
...
- Units are all Systeme International (SI) units as the [Microsoft.FSharp.Data.UnitSystems.SI.UnitNames Module (F#)](https://msdn.microsoft.com/visualfsharpdocs/conceptual/si.unitnames-namespace-%5bfsharp%5d) module defines.
<br />
...
<br />  You can use the [TypeProviderConfig](https://msdn.microsoft.com/visualfsharpdocs/conceptual/compilerservices.typeproviderconfig-class-%5bfsharp%5d) value that’s passed in to the type provider constructor to resolve relative file names.
<br />
