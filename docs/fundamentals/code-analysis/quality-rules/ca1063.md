...
- Modify Dispose() so that it calls Dispose(true), then calls <xref:System.GC.SuppressFinalize*> on the current object instance (`this`, or `Me` in Visual Basic), and then returns.
