...
- <xref:System.IEquatable`1.Equals*?displayProperty=fullName>
...
- <xref:System.Collections.IEqualityComparer.GetHashCode*?displayProperty=fullName>
...
The debugger uses <xref:System.Object.ToString*?displayProperty=fullName> to help display information about objects in string format. Therefore, `ToString` should not change the state of an object, and it shouldn't throw exceptions.
...
A <xref:System.IDisposable.Dispose*?displayProperty=fullName> method should not throw an exception. `Dispose` is often called as part of the cleanup logic in a `finally` clause. Therefore, explicitly throwing an exception from `Dispose` forces the user to add exception handling inside the `finally` clause.
