...
- An API marked with `[SupportedOSPlatform("platformName")]` is considered only portable to the specified OS platforms (the attribute can be applied multiple times with different platforms). For [**subset** platforms](../../../standard/analyzers/platform-compat-analyzer.md#platforms-relation), the attribute implies that the superset platform is also supported.
- An API marked with `[UnsupportedOSPlatform("platformName")]` is considered unsupported only to the specified OS platforms (the attribute can be applied multiple times with different platforms). For [**subset** platforms](../../../standard/analyzers/platform-compat-analyzer.md#platforms-relation), the attribute implies that the superset platform is also unsupported.
