...
- An API marked with `[SupportedOSPlatform("platformName")]` is considered only portable to the specified OS platforms (the attribute can be applied multiple times with different platforms). For [**related** platforms](../../../standard/analyzers/platform-compat-analyzer.md/#related-platforms) the attribute also imply its parent support.
- An API marked with `[UnsupportedOSPlatform("platformName")]` is considered unsupported only to the specified OS platforms (the attribute can be applied multiple times with different platforms). For [**related** platforms](../../../standard/analyzers/platform-compat-analyzer.md/#related-platforms) the attribute also imply its parent unsupport:
...
- If you access an API that's supported only on a specified platform (`[SupportedOSPlatform("platformName")]`) from code reachable on other platforms, you'll see the following violation: **'API' is supported on 'platformName'**.
  ...
  // An API supported on Windows, iOS from version 14.0 and MacCatalyst from version 14.0.
  [SupportedOSPlatform("windows")]
  [SupportedOSPlatform("ios14.0")] // MacCatalyst is superset of iOS, therefore it also supported on MacCatalyst 14.0
  ...
  public void Caller()
  {
      ...
      SupportedOnWindowsIos14AndMacCatalyst14(); // This call site is reachable on all platforms. 'SupportedOnWindowsAndIos14()'
...
- Suppress violations by surrounding the call site with the standard platform guard methods or custom guard APIs annotated with `SupportedOSPlatformGuardAttribute` or `UnsupportedOSPlatformGuardAttribute`.
  ...
  // An API supported on Windows, iOS from version 14.0, MacCatalyst from version 14.0.
  [SupportedOSPlatform("windows")]
  [SupportedOSPlatform("ios14.0")] // MacCatalyst is superset of iOS, therefore it also supported on MacCatalyst 14.0
  public void SupportedOnWindowsIos14AndMacCatalyst14() { }
