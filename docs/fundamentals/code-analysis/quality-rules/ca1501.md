...
By default, the rule excludes types from the `System` namespace. You can [configure](#configure-code-to-analyze) the rule to exclude other types or namespaces as well.
...
You can configure the inheritance tree depth at which this rule fires. For more information, [Configure threshold](#configure-threshold).
