...
By default, the rule only excludes types from the `System` namespace, but this is [configurable](#configure-code-to-analyze). You can also configure the threshold at which this rule fires. For more information, [Configure threshold](#configure-threshold).
...
2. Add the desired threshold to the text file in the following format:
   ...
   In this example, the rule is configured to fire when a type is more than eight levels deep in its inheritance hierarchy.
