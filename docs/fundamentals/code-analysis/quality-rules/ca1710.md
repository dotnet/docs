...
| <xref:System.Collections.Generic.IReadOnlyDictionary`2?displayProperty=fullName> | `Dictionary`                |
...
Types that implement <xref:System.Collections.IDictionary> or <xref:System.Collections.Generic.IReadOnlyDictionary`2> have names that end with the word `Dictionary` even if the type also implements <xref:System.Collections.IEnumerable> or <xref:System.Collections.ICollection>. The `Collection` and `Dictionary` suffix naming conventions enable users to distinguish between the following two enumeration patterns.
