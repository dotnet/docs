...
Strings are immutable and methods such as <xref:System.String.ToUpper*?displayProperty=nameWithType> return a new instance of a string instead of modifying the instance of the string in the calling method.
...
The following example shows a class that ignores the result of calling <xref:System.String.Trim*?displayProperty=nameWithType>.
...
The following example fixes the violation by assigning the result of <xref:System.String.Trim*?displayProperty=nameWithType> back to the variable it was called on.
