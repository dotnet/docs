...
Initializing a zero-length array leads to an unnecessary memory allocation. Instead, use the statically allocated empty array instance by calling the <xref:System.Array.Empty*?displayProperty=nameWithType> method. The memory allocation is shared across all invocations of this method.

## How to fix violations

To fix a violation, replace the zero-length array allocation with a call to <xref:System.Array.Empty*?displayProperty=nameWithType>. For example, the following two code snippets show a violation of the rule and how to fix it:
