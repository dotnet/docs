...
You can configure this rule to exclude the <xref:System.Linq.Enumerable.FirstOrDefault%2A?displayProperty=nameWithType> and <xref:System.Linq.Enumerable.LastOrDefault%2A?displayProperty=nameWithType> methods from analysis. You might consider excluding these methods if readability is a concern, since there's no *readable* replacement. To exclude these methods, add the following key-value pair to an *.editorconfig* file in your project:
