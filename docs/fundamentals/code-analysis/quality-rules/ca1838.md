...
When the P/Invoke is frequently called for larger buffers, <xref:System.Buffers.ArrayPool`1> can be used to avoid the repeated allocations and memory pressure that comes with them:
...
- <xref:System.Buffers.ArrayPool`1>
