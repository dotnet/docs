...
<xref:System.Linq.Enumerable.Any*?displayProperty=nameWithType> is called on a type that has a `Length`, `Count`, or `IsEmpty` *property*.

## Rule description

To determine whether a collection type has any elements, it's more efficient and clearer to use the `Length`, `Count`, or `IsEmpty` (if possible) properties than to call the <xref:System.Linq.Enumerable.Any*?displayProperty=nameWithType> method.
