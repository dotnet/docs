...
The following table summarizes the conditions for each rule.

| Diagnostic ID | Description | CodeFix |
| - | - | - |
| CA1865 | Applies when a safe transformation can be performed automatically with a CodeFix. | Yes |
| CA1866 | Applies when there's no specified comparison. | No |
| CA1867 | Applies for any other string comparison not covered by the other rules. | No |
