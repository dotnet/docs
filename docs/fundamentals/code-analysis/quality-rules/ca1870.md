...
Using a cached <xref:System.Buffers.SearchValues`1> instance is more efficient than passing values to `IndexOfAny` or `ContainsAny` directly.

## How to fix violations

Create and cache a <xref:System.Buffers.SearchValues`1> instance in a `static readonly` field, then pass that instance to the `IndexOfAny` or `ContainsAny` call instead.
