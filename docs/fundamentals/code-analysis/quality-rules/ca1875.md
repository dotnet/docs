...
'Regex.Count' is simpler and faster than 'Regex.Matches(...).Count'. The `Count` method is optimized for counting matches without materializing the full `MatchCollection`. Using `Matches` and then accessing `.Count` does unnecessary work that can impact performance.
