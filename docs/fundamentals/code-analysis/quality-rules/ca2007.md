...
To fix violations, call <xref:System.Threading.Tasks.Task.ConfigureAwait*> on the awaited <xref:System.Threading.Tasks.Task>. You can pass either `true` or `false` for the `continueOnCapturedContext` parameter.

- Calling `ConfigureAwait(true)` on the task has the same behavior as not explicitly calling <xref:System.Threading.Tasks.Task.ConfigureAwait*>. By explicitly calling this method, you're letting readers know you intentionally want to perform the continuation on the original synchronization context.
...
To fix the violation, call <xref:System.Threading.Tasks.Task.ConfigureAwait*> on the awaited <xref:System.Threading.Tasks.Task>:
