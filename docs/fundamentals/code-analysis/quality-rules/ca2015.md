...
Defining finalizers for types derived from <xref:System.Buffers.MemoryManager`1>

## Rule description

Adding a finalizer to a type derived from <xref:System.Buffers.MemoryManager`1> is likely an indication of a bug, as it suggests a native resource that could have been handed out in a <xref:System.Span`1> is getting cleaned up and potentially while it is still in use by the <xref:System.Span`1>.

> [!NOTE]
> The <xref:System.Buffers.MemoryManager`1> class is intended for advanced scenarios. Most developers do not need to use it.
