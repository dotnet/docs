...
A method sets the <xref:System.Data.IDbCommand.CommandText?displayProperty=fullName> property by using a string that's built from a string argument to the method.
...
The following .NET types implement the <xref:System.Data.IDbCommand.CommandText> property or provide constructors that set the property by using a string argument:
