...
To fix a violation of this rule, call <xref:System.IDisposable.Dispose*> on fields that are of types that implement <xref:System.IDisposable>.
...
- The call to <xref:System.IDisposable.Dispose*> occurs at a deeper calling level than the rule checks
...
The following snippet shows a type `TypeB` that violates rule CA2213 by declaring a field `aFieldOfADisposableType` as a disposable type (`TypeA`) and not calling <xref:System.IDisposable.Dispose*> on the field.
