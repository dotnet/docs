...
The result of a call to <xref:System.String.Compare*?displayProperty=nameWithType> is compared to zero.

## Rule description

<xref:System.String.Compare*?displayProperty=nameWithType> is designed to produce a total-order comparison that can be used for sorting. If you only care whether the strings are equal, it is both clearer and likely faster to use an equivalent overload of <xref:System.String.Equals*?displayProperty=nameWithType>.

## How to fix violations

To fix violations of this rule, replace the expression comparing the result of <xref:System.String.Compare*?displayProperty=nameWithType> with a call to <xref:System.String.Equals*?displayProperty=nameWithType>.
