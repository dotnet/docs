...
To create a multi-file file-based program, start the entry point file with a shebang (`#!`) line to clearly distinguish it from files brought in with `#:include`. This helps tools identify the entry point correctly, and improves direct shell execution support.
...
It is safe to suppress this warning if you intentionally omit the shebang line and have confirmed that your tooling can still identify the entry point correctly.
