...
You can potentially suppress this warning if the <xref:System.Xml.Schema.XmlSchema.Read*?displayProperty=nameWithType> method is always used to process XML that comes from a trusted source and hence cannot be tampered with.
...
The type of the first parameter of <xref:System.Xml.Schema.XmlSchema.Read*?displayProperty=nameWithType> is not `XmlReader`.
