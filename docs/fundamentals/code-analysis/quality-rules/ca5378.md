...
A <xref:System.AppContext.SetSwitch*?displayProperty=nameWithType> method call sets `Switch.System.ServiceModel.DisableUsingServicePointManagerSecurityProtocols` to `true`.
...
- If your application targets .NET Framework v4.7 or later, you can either remove the <xref:System.AppContext.SetSwitch*?displayProperty=nameWithType> method call, or set the switch's value to `false`.
