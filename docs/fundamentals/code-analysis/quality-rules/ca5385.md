...
- Using <xref:System.Security.Cryptography.AsymmetricAlgorithm.Create*?displayProperty=fullName> without parameter which would create RSA with the default key size 1024.
- Using <xref:System.Security.Cryptography.AsymmetricAlgorithm.Create*?displayProperty=fullName> and specifying the `algName` parameter as `RSA` with the default key size 1024.
- Using <xref:System.Security.Cryptography.CryptoConfig.CreateFromName*?displayProperty=fullName> and specifying the `name` parameter as `RSA` with the default key size 1024.
- Using <xref:System.Security.Cryptography.CryptoConfig.CreateFromName*?displayProperty=fullName> and specifying the `name` parameter as `RSA` and specifying the key size as smaller than 2048 explicitly by `args`.
