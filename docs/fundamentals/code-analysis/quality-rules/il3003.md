...
Here is a list of possible scenarios where the warning can be generated:
...
For all interfaces and overrides the implementation 'RequiresAssemblyFilesAttribute' must match the definition 'RequiresAssemblyFilesAttribute', either all the members contain the attribute or none of them. Remove or add the attribute on the interface/base class members or implementing/deriving class so that the attribute is on both or neither.
