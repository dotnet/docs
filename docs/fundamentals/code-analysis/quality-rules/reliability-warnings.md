...
| [CA2026: Prefer JsonElement.Parse over JsonDocument.Parse().RootElement](ca2026.md) | It's more efficient to use <xref:System.Text.Json.JsonElement.Parse%2A?displayProperty=nameWithType> directly than to use `JsonDocument.Parse(json).RootElement`. |
