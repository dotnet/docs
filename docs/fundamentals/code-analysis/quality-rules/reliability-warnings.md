...
|[CA2018: Buffer.BlockCopy expects the number of bytes to be copied for the count argument](ca2018.md) | `Buffer.BlockCopy` expects the number of bytes to be copied for the `count` argument. Using `Array.Length` may not match the number of bytes that needs to be copied. |
