...
| **Option values** | `discard_variable` - Prefer to use a [discard](/dotnet/csharp/fundamentals/functional/discards) when assigning a value that's not used<br /><br />`unused_local_variable` - Prefer to use a local variable when assigning a value that's not used |
