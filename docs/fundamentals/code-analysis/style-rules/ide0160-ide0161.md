...
These rules concern how namespaces are declared. `IDE0160` is reported when file-scoped namespaces are used. and `IDE0161` is reported when block-scoped namespaces are used.
