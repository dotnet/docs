...
These rules flag `async` methods that don't contain any `await` expressions. An `async` method that never `await`s anything is unnecessary overhead—it runs synchronously but with the overhead of the async state machine. Such methods can be simplified by removing the `async` modifier and adjusting the return type.
