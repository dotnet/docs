...
- [Metrics](metrics.md): Starting with .NET 8.0, the HTTP and the name resolution (DNS) components are instrumented using the modern, OpenTelemetry-compatible Metrics API-s.
- [Event Counters](event-counters.md): All Networking components are instrumented to publish real-time performance metrics using the EventCounters API-s.
- [Events](events.md): Debug/trace information with accurate timestamps.
