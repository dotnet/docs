...
When the result of the division and multiplication is passed to the <xref:System.Math.Round*> method, the result suffers no loss of precision, as the following code shows.
...
The <xref:System.Decimal> type supports standard mathematical operations such as addition, subtraction, division, multiplication, and unary negation. You can also work directly with the binary representation of a <xref:System.Decimal> value by calling the <xref:System.Decimal.GetBits*> method.

To compare two <xref:System.Decimal> values, you can use the standard numeric comparison operators, or you can call the <xref:System.Decimal.CompareTo*> or <xref:System.Decimal.Equals*> method.
