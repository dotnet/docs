...
In desktop apps, if `culture` is `null`, the <xref:System.Resources.ResourceManager.GetString(System.String,System.Globalization.CultureInfo)> method uses the current UI culture obtained from the <xref:System.Globalization.CultureInfo.CurrentUICulture*?displayProperty=nameWithType> property.
