...
In cases where a loss of precision is likely to affect the result of a comparison, you can use the following techniques instead of calling the <xref:System.Single.Equals*> or <xref:System.Single.CompareTo*> method:
...
- **Comparison of values**. You can call the <xref:System.Single.Equals*> method to determine whether two <xref:System.Single> values are equal, or the <xref:System.Single.CompareTo*> method to determine the relationship between two values.
  ...
  You can also call the <xref:System.Single.IsNaN*>, <xref:System.Single.IsInfinity*>, <xref:System.Single.IsPositiveInfinity*>, and <xref:System.Single.IsNegativeInfinity*> methods to test for these special values.
