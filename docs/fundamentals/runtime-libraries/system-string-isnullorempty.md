...
A string is empty if it's explicitly assigned an empty string ("") or <xref:System.String.Empty?displayProperty=nameWithType>. An empty string has a <xref:System.String.Length> of 0. The following example creates an empty string and displays its value and its length.
