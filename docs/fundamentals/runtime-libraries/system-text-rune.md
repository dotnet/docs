...
* <xref:System.Char.GetNumericValue*?displayProperty=nameWithType>
* <xref:System.Char.GetUnicodeCategory*?displayProperty=nameWithType>
* <xref:System.Char.IsDigit*?displayProperty=nameWithType>
* <xref:System.Char.IsLetter*?displayProperty=nameWithType>
* <xref:System.Char.IsLetterOrDigit*?displayProperty=nameWithType>
* <xref:System.Char.IsLower*?displayProperty=nameWithType>
* <xref:System.Char.IsNumber*?displayProperty=nameWithType>
* <xref:System.Char.IsPunctuation*?displayProperty=nameWithType>
* <xref:System.Char.IsSymbol*?displayProperty=nameWithType>
* <xref:System.Char.IsUpper*?displayProperty=nameWithType>
...
* <xref:System.Char.IsSurrogate*?displayProperty=nameWithType>
* <xref:System.Char.IsSurrogatePair*?displayProperty=nameWithType>
* <xref:System.Char.IsHighSurrogate*?displayProperty=nameWithType>
* <xref:System.Char.IsLowSurrogate*?displayProperty=nameWithType>
* <xref:System.Char.ConvertFromUtf32*?displayProperty=nameWithType>
* <xref:System.Char.ConvertToUtf32*?displayProperty=nameWithType>
...
There are <xref:System.Text.Rune.TryCreate*?displayProperty=nameWithType> methods available for callers who don't want exceptions to be thrown on failure.

`Rune` instances can also be read from existing input sequences. For instance, given a `ReadOnlySpan<char>` that represents UTF-16 data, the <xref:System.Text.Rune.DecodeFromUtf16*?displayProperty=nameWithType> method returns the first `Rune` instance at the beginning of the input span. The <xref:System.Text.Rune.DecodeFromUtf8*?displayProperty=nameWithType> method operates similarly, accepting a `ReadOnlySpan<byte>` parameter that represents UTF-8 data. There are equivalent methods to read from the end of the span instead of the beginning of the span.
...
Many of the static APIs available on the `char` type are also available on the `Rune` type. For instance, <xref:System.Text.Rune.IsWhiteSpace*?displayProperty=nameWithType> and <xref:System.Text.Rune.GetUnicodeCategory*?displayProperty=nameWithType> are equivalents to <xref:System.Char.IsWhiteSpace*?displayProperty=nameWithType> and <xref:System.Char.GetUnicodeCategory*?displayProperty=nameWithType> methods. The `Rune` methods correctly handle surrogate pairs.
...
* The <xref:System.Text.Rune.GetUnicodeCategory*?displayProperty=nameWithType> doesn't always return the same result as <xref:System.Char.GetUnicodeCategory*?displayProperty=nameWithType>. It does return the same value as <xref:System.Globalization.CharUnicodeInfo.GetUnicodeCategory*?displayProperty=nameWithType>. For more information, see the **Remarks** on <xref:System.Char.GetUnicodeCategory*?displayProperty=nameWithType>.
...
The <xref:System.Text.Rune.EncodeToUtf16*?displayProperty=nameWithType> converts a `Rune` instance to `char` instances. To query the number of `char` instances that would result from converting a `Rune` instance to UTF-16, use the <xref:System.Text.Rune.Utf16SequenceLength?displayProperty=nameWithType> property. Similar methods exist for UTF-8 conversion.
...
The <xref:System.Text.Rune.EncodeToUtf16*?displayProperty=nameWithType> and <xref:System.Text.Rune.EncodeToUtf8*?displayProperty=nameWithType> methods return the actual number of elements written. They throw an exception if the destination buffer is too short to contain the result. There are non-throwing <xref:System.Text.Rune.TryEncodeToUtf8*> and <xref:System.Text.Rune.TryEncodeToUtf16*> methods as well for callers who want to avoid exceptions.
