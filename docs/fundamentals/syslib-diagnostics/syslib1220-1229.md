...
|  `SYSLIB1225` | Type includes a ref-like property, a field, or a constructor parameter. |
|  `SYSLIB1226` | `JsonIgnoreCondition.Always` is not valid on type-level `JsonIgnoreAttribute` annotations. |
|  `SYSLIB1227` | Union case types cannot be unambiguously classified by JSON value type. |
|  `SYSLIB1228` | Union type shape is not a valid C# union. |
|  `SYSLIB1229` | Open generic derived type could not be resolved for the polymorphic base type. |
