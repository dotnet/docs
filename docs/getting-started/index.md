...
.NET is a general purpose development platform. It is used for many kinds of apps and workloads. It has several key features that make it easy and efficient to build great applications, including automatic memory management and modern programming languages. .NET enables a high-level programming environment with many convenience features, while providing low-level access to native memory and APIs.
