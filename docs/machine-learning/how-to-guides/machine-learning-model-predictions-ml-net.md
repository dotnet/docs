...
Here, the `HousingPrediction` data model inherits from `HousingData` to make it easy to visualize the original input data along with the output generated by the model. But the data model does not have to inherit from the input model, especially in cases where the input data is being transformed during the training, so the output model no longer represents the data types.  
