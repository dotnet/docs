...
- Future: Other ML Tasks such as `recommendation`, `anomaly-detection`, `clustering`, etc. will be supported in future releases
...
The third asset, the training code, shows you what ML.NET API code was used by the CLI to train the generated model, so you can investigate what specific trainer/algorithm and hyper-paramenters were selected by the CLI and [AutoML](../AutoML/README.MD) under the covers. 


## The 'auto-train' command uses the AutoML engine

The CLI uses the ML.NET AutoML engine (NuGet package) under the covers in order to intelligently search for the best quality models, as shown in the following diagram:

![image](./media/ml-net-automl-working-diagram.png "AutoML engine working inside the ML.NET CLI")

This is why when running the ML.NET CLI tool with the `auto-train- command you will see the tool trying many iterations with different algorithms and combinations of configuration.
...
Specify a string, between quotes, with the column names you want to ignore. Use ',' (comma) to separate multiple column names.
...
Currently, the by default maximum exploration time is 10 seconds.
...
If you use cache it means that the whole training dataset will be loaded in-memory.

For small and medium datasets, using cache can drastically improve the training performance, meaning the traing time can be significantly shorter than when you don't use cache.

However, for very large datasets, loading all the data in memory can impact negatively since you might get out of memory. When training with very large dataset files and not using cache, ML.NET will be streaming chuncks of data from the drive when it nees to load more data while training.

You can specify the following values:

`on`: Forces cache to be used when training. 
`off`: Forces cache not to be used when training.
`auto`: Depending on AutoML heuristics cache will be used or not. Usually, small/medium datasets will use cache and very large datasets won't use cache if you use the `auto` choice.
...
- [ML.NET Command-Line Interface (CLI) tool overview and installation](http://TBD)
