...
The machine learning task used to train the sentiment analysis model in this tutorial is image classification. During the model training process, Model Builder trains separate models using ResNet50 algorithm and settings to find the best performing model for your dataset.
...
1. In the code step of the Model Builder tool, select **Add Projects** to add the autogenerated projects to the solution.
    ...
    - **LandUseML.ConsoleApp**: A .NET Core Console application that contains the model training and prediction code.
    - **LandUSeML.Model**: A .NET Standard class library containing the data models that define the schema of input and output model data as well as the saved version of the best performing model during training.

    For this tutorial, only the **LandUseML.Model** project is used because predictions will be made in the SentimentRazor web application rather than in the console.
