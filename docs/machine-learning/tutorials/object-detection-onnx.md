...
Training an object detection model from scratch requires setting millions of parameters, a ton of labeled training data and a vast amount of compute resources (hundreds of GPU hours). Using a pre-trained model allows you to shortcut this process.
...
- [Microsoft.ML NuGet Package 1.1.0](https://www.nuget.org/packages/Microsoft.ML/)
- [Microsoft.ML.ImageAnalytics 1.1.0 NuGet Package](https://www.nuget.org/packages/Microsoft.ML.ImageAnalytics/)
...
**PLACEHOLDER**: DL, YOLO, DNN info here.
...
1. Copy the `assets` directory into your *ObjectDetection* project directory. This directory and its subdirectories contain the data and support files (except for the Tiny YOLOv2 model, which you'll download and add in the next step) needed for this tutorial.

1. Download the [Tiny YOLOv2 model](https://storage.googleapis.com/download.tensorflow.org/models/inception5h.zip), and unzip.
...
Next, define the paths of the various assets. Start by adding the `GetAbsolutePath` method below the `Main` method in the `Program` class. 
...
Then, inside the `Main` method, create fields to store the location of your assets:
...
1. In **Solution Explorer**, right-click the project, and then select **Add** > **New Item**. When the new folder appears in the Solution Explorer, name it "DataStructures".
...
Now, create your prediction class in the *DataStructures* directory.
...
1. In the **Add New Item** dialog box, select **Class** and change the **Name** field to *ImageNetPrediction.cs*. Then, select the **Add** button.
    ...
    - `PredictedLabel` contains the dimensions, objectness score and class probabilities for all each of the bounding boxes detected in an image.
...
After the model has been scored and the outputs have been processed, the bounding boxes have to be on the image. To do so, add a method called `DrawBoundingBox` below the `GetAbsolutePath` method insode of *Program.cs*.
...
Then, create a foreach loop to iterate over each of the bounding boxes.
...
Inside of the foreach loop, get the dimensions of the bounding box.
...
Because the dimensions of the bounding box correspond to the model input of `416 * 416`, scale the bounding box dimensions to match the actual size of the image.
...
Then, define what the text that appears on the image looks like.

```csharp
string text = $"{box.Label} ({(box.Confidence * 100).ToString("0")}%)";
```

The image cannot be drawn on in its current format. To draw on it, convert it to a `Graphic`.
...
In the `using` statement, tune the graphic's settings.
...
Below that, define the text and bounding box options.
...
Draw the text and bounding box on the image using the `DrawString` and `DrawRectangle` methods.
...
Outside of the foreach loop, add code to save the images in the `outputDirectory`.
...
The expected output generated by the pre-trained ONNX model is a tensor of shape `125 * 13 * 13` that is returned as a one-dimensional `float[]` of 21125 elements. In order to extract the predictions generated by the model, some post-processing work is required. To do so, create a set of classes to help parse the output.

Add a new directory to your project to store your parser classes.

1. In **Solution Explorer**, right-click the project, and then select **Add** > **New Item**. When the new folder appears in the Solution Explorer, name it "YoloParser".
...
1. In the **Add New Item** dialog box, select **Class** and change the **Name** field to *DimensionsBase.cs*. Then, select the **Add** button.
    ...
    The *ImageNetPrediction.cs* file opens in the code editor. Remove all `using` statements.

    Remove the existing class definition and add the following code for the `DimensionsBase` class to the *DimensionsBase.cs* file:
    ...
    - `Y` contains the position of the object along the x-axis.
...
1. In the **Add New Item** dialog box, select **Class** and change the **Name** field to *YoloBoundingBox.cs*. Then, select the **Add** button.
    ...
    Just above the existing class definition, add a new class called `BoundingBoxDimensions` which inherits from the `DimensionsBase` class.
    ...
    - `BoxColor` contains the color associated with the respective class.

### Create parser

Now that the classes for dimensions and bounding boxes are created, it's time to create the parse which will utilize the recently created classes.
...
- CHANNEL_COUNT is the total number of values contained in on cell of the grid.
...
Next, define the labels or classes that the model will predict. This model predicts 20 classes which is a subset of the total number of classes predicted by the original model.

Add your list of labels below your `anchors`. 
...
Add the following helper methods to the `YoloOutputParser` class are:

- Sigmoid applies the sigmoid function that outputs a number between 0 and 1.
- Softmax normalizes input vector into a probability distribution.
- GetOffset maps elements in the one-dimensional model output to the corresponding position in a `125 * 13 * 13` tensor.
...
Below the `IntersectionOverUnion` method, create the `ParseOutputs` method.
...
Create a list to hold your bounding boxes and define variables inside the `ParseOutputs` method

```csharp
var boxes = new List<YoloBoundingBox>();
```

Each image is divided into a grid of `13 * 13` cells. Each cell contains five boxes. Below the `boxes` variable, add code to process all of the boxes in each of the cells.
...
Inside the inner-most loop, calculate the starting position of the current box.
...
Then, use the `GetConfidence` method to get the confidence for the current box.
...
Otherwise, continue processing the output. The next step is to get the probability distribution of the predicted classes for the current box using the `ExtractClasses` method.
...
Now that you have all of the highly confident bounding boxes have been extracted from the model output, additional filtering needs to be done to remove overlapping images. Add a method called `FilterBoundingBoxes` below the `ParseOutputs` method:
...
Otherwise, look at the adjacent bounding boxes. Add the following code below the limit check.
...
1. In the **Add New Item** dialog box, select **Class** and change the **Name** field to *OnnxModelScorer.cs*. Then, select the **Add** button.
    ...
    After that, create another struct called `TinyYoloModelSettings` which contains the names of the input and output layers of the model.
    ...
    ML.NET pipelines typically expect data to operate on when the `Fit` method is called. In this case, a process similar to training will be used. However, because no actual training is happening, it is acceptable to use an empty `IDataView`. Create a new IDataView for the pipeline.
    ...
    - LoadImages loads the image as Bitmap
    ...
    Define your pipeline in the `LoadModel` method.
...
Now it's time for the post-processing step. Create an instance of `YoloOutputParser` and use it to post-process the model output.
