...
1. Add the following using statements to reference :
...
1. Add the `ConsumeModel` method to the `Program` class. The `ConsumeModel` will create a [`PredictionEngine`](xref:Microsoft.ML.PredictionEngine%602) based on the model generated by Model Builder to make predictions on new data and print them out to the console.
...
7. To make a prediction on new data using the model, add the following code to the `Main` method and run the application.
