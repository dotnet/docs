...
Orleans applications are designed to scale up and scale out efficiently. To accomplish this, instances of your application communicate directly with each other via TCP sockets and therefore Orleans requires network connectivity communication between silo. Azure App Service supports this via [virtual network integration](https://learn.microsoft.com/azure/app-service/overview-vnet-integration) with configuration instructing App Service to allocate private network ports for your app instances.
