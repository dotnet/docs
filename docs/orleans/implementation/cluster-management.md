...
1. `ZooKeeper`: membership table is stored in a ZooKeeper [ensemble](http://zookeeper.apache.org/doc/r3.4.6/zookeeperAdmin.html#sc_zkMulitServerSetup).
...
A natural question that might be asked is why not rely completely on [Apache ZooKeeper](https://ZooKeeper.apache.org/) for the cluster membership implementation, potentially by using its out of the box support for [group membership with ephemeral nodes] (<http://zookeeper.apache.org/doc/trunk/recipes.html#sc_outOfTheBox>)? Why did we bother implementing our membership protocol? There were primarily three reasons:
