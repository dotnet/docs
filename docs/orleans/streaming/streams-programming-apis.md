...
The consumer of a rewindable stream can pass a <xref:Orleans.Streams.StreamSequenceToken> to the `SubscribeAsync` call. The runtime delivers events starting from that <xref:Orleans.Streams.StreamSequenceToken>. A null token means the consumer wants to receive events starting from the latest.
