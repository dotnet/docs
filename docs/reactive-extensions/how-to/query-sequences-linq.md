...
description: Learn how to query observable sequences using various LINQ operators with Reactive Extensions for .NET.
author: IEvangelist
ms.date: 11/09/2020
ms.author: dapine
ms.topic: how-to
...
In this article, you'll learn about the first-class nature of observable sequences, in which generic LINQ operators are available to manipulate observable objects. Most operators take an observable sequence, and perform some logic on it, and output another observable sequence. In addition, you can chain multiple operators on a source sequence to tweak the resulting sequence to your exact requirement.

## Operators

The `Create` and `Generate` operators are used to create and return simple sequences. The `FromEventPattern` operator is used to convert existing .NET events into observable sequences. There are additional static LINQ operators on the `Observable` type that expose filtering, grouping, and transforming data. Such operators take observable sequence(s) as input, and produce observable sequence(s) as output.

### Combine sequences

In this section, you'll learn some of the operators that combine various observable sequences into a single observable sequence. The data is *not* transformed when you combine sequences.
...
The Select operator can translate each element of an observable sequence into another form. In the following example, we project a sequence of integers into strings of length `n` respectively.
...
seqString.Subscribe(Console.WriteLine);

Console.ReadKey();
```

In the following sample, you use the `Select` operator to project the `IEventPattern<MouseEventArgs>` data type into a `Point` type. You are transforming the mouse move events into a sequence that can be parsed, and manipulated further.
...
```csharp
IObservable<int> sequence =
    Observable.Generate(0, i => i < 10, i => i + 1, i => i * i);
...
> [!NOTE]
> If you are using `Buffer` or `Window`, you have to make sure that the sequence is not empty before filtering on it.
