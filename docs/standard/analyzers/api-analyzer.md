...
To suppress warnings locally, right-click on the member you want to suppress warnings for and then select **Quick Actions and Refactorings** > **Suppress *diagnostic ID*\<diagnostic ID>** > **in Source**. The [#pragma](/dotnet/csharp/language-reference/preprocessor-directives) warning preprocessor directive is added to your source code in the scope defined:
