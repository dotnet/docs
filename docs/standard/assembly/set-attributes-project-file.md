...
- `SourceRevisionId` is appended to `InformationalVersion`. Starting in .NET 8, the .NET SDK automatically populates `SourceRevisionId` with the commit hash when [Source Link](https://github.com/dotnet/sourcelink) is present. You can disable this behavior by setting `IncludeSourceRevisionInInformationalVersion` to `false`.
