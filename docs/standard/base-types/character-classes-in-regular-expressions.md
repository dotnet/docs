...
- A non-word character. A character in the input string can belong to any Unicode category that isn't a word character. For more information, see [Non-Word Character](#NonWordCharacter).

- A white-space character. A character in the input string can be any Unicode separator character and any one of many control characters. For more information, see [White-Space Character](#WhitespaceCharacter).

- A non-white-space character. A character in the input string can be any character that isn't a white-space character. For more information, see [Non-White-Space Character](#NonWhitespaceCharacter).

- A decimal digit. A character in the input string can be any character classified as a Unicode decimal digit. For more information, see [Decimal Digit Character](#DigitCharacter).
...
A positive character group specifies a list of characters, any one of which can appear in an input string for a match to occur. This list of characters might be specified individually, as a range, or both.
...
> [!NOTE]
> Because a positive character group can include both a set of characters and a character range, a hyphen character (`-`) is always interpreted as the range separator unless it's the first or last character of the group.
...
:::code language="vb" source="snippets/character-classes-in-regular-expressions/vb/Program.vb" id="PositiveCharacterGroup":::
...
:::code language="vb" source="snippets/character-classes-in-regular-expressions/vb/Program.vb" id="CharacterRange":::
...
A negative character group specifies a list of characters that must not appear in an input string for a match to occur. The list of characters can be specified individually, as a range, or both.

The syntax for specifying a list of individual characters is as follows:

`[*^character_group*]`

Where *character_group* is a list of the individual characters that can't appear in the input string for a match to succeed. *character_group* can consist of any combination of one or more literal characters, [escape characters](character-escapes-in-regular-expressions.md), or character classes.
...
> [!NOTE]
> Because a negative character group can include both a set of characters and a character range, a hyphen character (`-`) is always interpreted as the range separator unless it's the first or last character of the group.
...
> [!IMPORTANT]
> A negative character group in a larger regular expression pattern isn't a zero-width assertion. That is, after evaluating the negative character group, the regular expression engine advances one character in the input string.
...
The following example matches any word that begins with the characters "th" and isn't followed by an "o".
...
:::code language="vb" source="snippets/character-classes-in-regular-expressions/vb/Program.vb" id="NegativeCharacterGroup":::
...
|`[^o]`|Match any character that isn't an "o".|
...
- In a positive or negative character group, a period is treated as a literal period character, and not as a character class. For more information, see [Positive Character Group](#PositiveGroup) and [Negative Character Group](#NegativeGroup) earlier in this article. The following example provides an illustration by defining a regular expression that includes the period character (`.`) both as a character class and as a member of a positive character group. The regular expression `\b.*[.?!;:](\s|\z)` begins at a word boundary, matches any character until it encounters one of five punctuation marks, including a period, and then matches either a white-space character or the end of the string.
  ...
  :::code language="vb" source="snippets/character-classes-in-regular-expressions/vb/Program.vb" id="AnyCharacterSingleline":::
...
The Unicode standard assigns each character a general category. For example, a particular character might be an uppercase letter (represented by the `Lu` category), a decimal digit (the `Nd` category), a math symbol (the `Sm` category), or a paragraph separator (the `Zl` category). Specific character sets in the Unicode standard also occupy a specific range or block of consecutive code points. For example, the basic Latin character set is found from \u0000 through \u007F, while the Arabic character set is found from \u0600 through \u06FF.
...
:::code language="vb" source="snippets/character-classes-in-regular-expressions/vb/Program.vb" id="UnicodeCategory":::
...
:::code language="vb" source="snippets/character-classes-in-regular-expressions/vb/Program.vb" id="NegativeUnicodeCategory":::

The regular expression pattern `(\P{Sc})+` matches one or more characters that aren't currency symbols; it effectively strips any currency symbol from the result string.
...
:::code language="vb" source="snippets/character-classes-in-regular-expressions/vb/Program.vb" id="WordCharacter":::
...
`\s` matches any whitespace character. It's equivalent to the escape sequences and Unicode categories listed in the following table.
...
:::code language="vb" source="snippets/character-classes-in-regular-expressions/vb/Program.vb" id="NonWhitespaceCharacter":::

<a name="DigitCharacter"></a>

## Decimal digit character: \d

`\d` matches any decimal digit. It's equivalent to the `\p{Nd}` regular expression pattern, which includes the standard decimal digits 0-9 and the decimal digits of many other character sets.
...
:::code language="vb" source="snippets/character-classes-in-regular-expressions/vb/Program.vb" id="DigitCharacter":::

<a name="NonDigitCharacter"></a>

## Non-digit character: \D

`\D` matches any non-digit character. It's equivalent to the `\P{Nd}` regular expression pattern.
...
:::code language="vb" source="snippets/character-classes-in-regular-expressions/vb/Program.vb" id="NonDigitCharacter":::
...
|`Pi`|Punctuation, Initial quote (might behave like Ps or Pe depending on usage)|
|`Pf`|Punctuation, Final quote (might behave like Ps or Pe depending on usage)|
...
Choose character classes for a character class subtraction expression that yield useful results. Avoid an expression that yields an empty set of characters, which can't match anything, or an expression that is equivalent to the original base group. For example, the empty set is the result of the expression `[\p{IsBasicLatin}-[\x00-\x7F]]`, which subtracts all characters in the `IsBasicLatin` character range from the `IsBasicLatin` general category. Similarly, the original base group is the result of the expression `[a-z-[0-9]]`.  This is because the base group, which is the character range of letters from "a" through "z", doesn't contain any characters in the excluded group, which is the character range of decimal digits from "0" through "9".
...
:::code language="vb" source="snippets/character-classes-in-regular-expressions/vb/Program.vb" id="CharacterClassSubtraction":::
