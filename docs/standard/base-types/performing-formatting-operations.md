---
title: Performing formatting operations
description: Performing formatting operations
keywords: .NET, .NET Core
author: stevehoag
manager: wpickett
ms.date: 07/25/2016
ms.topic: article
ms.prod: .net-core

ms.devlang: dotnet
ms.assetid: 982f0b64-7e69-4bbe-a688-0117f86a603c
---

# Performing formatting operations

The following topics provide step-by-step instructions for performing specific formatting operations.

* [How to: pad a number with leading zeros](pad-number.md)

* [How to: define and use custom numeric format providers](define-custom.md)

* [How to: extract the day of the week from a specific date](extract-day.md)

* [How to: round-trip date and time values](roundtrip.md)

* [How to: display milliseconds in date and time values](display-milliseconds.md)

* [How to: display dates in non-gregorian calendars](display-dates.md)

## See Also

[Formatting types](formatting-types.md)
