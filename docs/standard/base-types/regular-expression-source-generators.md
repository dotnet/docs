...
`Regex.CompileToAssembly` itself has problems, however. First, it's not user friendly. Because a utility was required that called `CompileToAssembly` to produce an assembly your app would reference, there's relatively little use of this otherwise valuable feature. On .NET [Core], `CompileToAssembly` has never been supported, as it requires the ability to save reflection-emit code to assemblies on disk, which also isn't supported. This is where source generation becomes valuable.
