...
### Example 3: Dynamically build a culture-sensitive regular expression  
 ...
 The following example illustrates the power of regular expressions combined with the flexibility offered by .NET's globalization features. It uses the <xref:System.Globalization.NumberFormatInfo> object to determine the format of currency values in the en-US culture. It then uses that information to dynamically construct a regular expression that extracts currency values from the text. For each match, it extracts the subgroup that contains the numeric string only, converts it to a <xref:System.Decimal> value, and calculates a running total. While this example explicitly uses the en-US culture, you can apply the same technique with any culture by substituting a different <xref:System.Globalization.CultureInfo> object—the regular expression adapts automatically to that culture's currency symbols and separators.
