...
- **Supports side-by-side .NET versions per application.**
  ...
  Side-by-side installation isn't possible with .NET Framework 4. It's a Windows component, and only one version of .NET Framework 4 can exist on a machine at a time (.NET Framework 3.5 is separate and optional): each version of .NET Framework 4 replaces the previous version.
