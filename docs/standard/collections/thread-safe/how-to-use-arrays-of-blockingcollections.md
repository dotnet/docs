...
The following example shows how to use arrays of <xref:System.Collections.Concurrent.BlockingCollection`1?displayProperty=nameWithType> objects with static methods such as <xref:System.Collections.Concurrent.BlockingCollection`1.TryAddToAny*> and <xref:System.Collections.Concurrent.BlockingCollection`1.TryTakeFromAny*> to implement fast and flexible data transfer between components.
