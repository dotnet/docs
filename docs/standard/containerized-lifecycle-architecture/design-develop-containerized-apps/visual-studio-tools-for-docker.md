...
If you have multiple projects in a solution that each have an associated container, you should consider adding Docker Compose functionality.  To add this functionality, right-click on the solution or project node, and choose **Add > Container Orchestration Support**.
...
You could deploy a multicontainer application by using a single *docker-compose.yml* file when you run 1docker-compose up`; however, Visual Studio adds a group of them, so you can override values depending on the environment (development versus production) and the execution type (release versus debug). This capability will be better explained in later chapters.
