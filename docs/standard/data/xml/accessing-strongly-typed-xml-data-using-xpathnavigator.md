...
|--------------|-----------------|
|<xref:System.Xml.XPath.XPathNavigator.XmlType*>|This contains the XML schema type information for the node if it is valid.|
|<xref:System.Xml.XPath.XPathNavigator.SchemaInfo*>|This contains the Post Schema Validation Infoset of the node that is added after validation. This includes the XML schema type information, as well as validity information.|
|<xref:System.Xml.XPath.XPathNavigator.ValueType*>|The CLR type of the typed value of the node.|
|<xref:System.Xml.XPath.XPathNavigator.TypedValue*>|The content of the node as one or more CLR values whose type is the closest match to the XML schema type of the node.|
|<xref:System.Xml.XPath.XPathNavigator.ValueAsBoolean*>|The <xref:System.String> value of the current node cast to a <xref:System.Boolean> value, according to the XPath 2.0 casting rules for `xs:boolean`.|
|<xref:System.Xml.XPath.XPathNavigator.ValueAsDateTime*>|The <xref:System.String> value of the current node cast to a <xref:System.DateTime> value, according to the XPath 2.0 casting rules for `xs:datetime`.|
|<xref:System.Xml.XPath.XPathNavigator.ValueAsDouble*>|The <xref:System.String> value of the current node cast to a <xref:System.Double> value, according to the XPath 2.0 casting rules for `xsd:double`.|
|<xref:System.Xml.XPath.XPathNavigator.ValueAsInt*>|The <xref:System.String> value of the current node cast to a <xref:System.Int32> value, according to the XPath 2.0 casting rules for `xs:integer`.|
|<xref:System.Xml.XPath.XPathNavigator.ValueAsLong*>|The <xref:System.String> value of the current node cast to a <xref:System.Int64> value, according to the XPath 2.0 casting rules for `xs:integer`.|
|<xref:System.Xml.XPath.XPathNavigator.ValueAs*>|The contents of the node cast to the target type according to the XPath 2.0 casting rules.|
...
1. Validation Outcomes: Information as to whether an element or attribute was successfully validated or not. This is exposed by the <xref:System.Xml.Schema.IXmlSchemaInfo.Validity> property of the <xref:System.Xml.XPath.XPathNavigator.SchemaInfo> property of the <xref:System.Xml.XPath.XPathNavigator> class.

2. Default Information: Indications as to whether the value of the element or attribute was obtained via default values specified in the schema or not. This is exposed by the <xref:System.Xml.Schema.IXmlSchemaInfo.IsDefault> property of the <xref:System.Xml.XPath.XPathNavigator.SchemaInfo> property of the <xref:System.Xml.XPath.XPathNavigator> class.

3. Type Annotations: References to schema components that may be type definitions or element and attribute declarations. The <xref:System.Xml.XPath.XPathNavigator.XmlType> property of the <xref:System.Xml.XPath.XPathNavigator> contains the specific type information of the node if it is valid. If the validity of a node is unknown, such as when it was validated then subsequently edited. then the <xref:System.Xml.XPath.XPathNavigator.XmlType> property is set to `null` but type information is still available from the various properties of the <xref:System.Xml.XPath.XPathNavigator.SchemaInfo> property of the <xref:System.Xml.XPath.XPathNavigator> class.
...
## Obtain Typed Values Using ValueAs Properties
 ...
 The typed value of a node can be retrieved by accessing the <xref:System.Xml.XPath.XPathNavigator.TypedValue> property of the <xref:System.Xml.XPath.XPathNavigator>. In certain cases you may want to convert the typed value of a node to a different type. A common example is to get a numeric value from an XML node. For example, consider the following unvalidated and untyped XML document.
...
```
 ...
 If the <xref:System.Xml.XPath.XPathNavigator> is positioned on the `price` element the <xref:System.Xml.XPath.XPathNavigator.XmlType> property would be `null`, the <xref:System.Xml.XPath.XPathNavigator.ValueType> property would be <xref:System.String>, and the <xref:System.Xml.XPath.XPathNavigator.TypedValue> property would be the string `10.00`.

 However, it is still possible to extract the value as a numeric value using the <xref:System.Xml.XPath.XPathItem.ValueAs*>, <xref:System.Xml.XPath.XPathNavigator.ValueAsDouble*>, <xref:System.Xml.XPath.XPathNavigator.ValueAsInt*>, or <xref:System.Xml.XPath.XPathNavigator.ValueAsLong*> method and properties. The following example illustrates performing such a cast using the <xref:System.Xml.XPath.XPathItem.ValueAs*> method.
