...
|Execute the style sheet.|Resolve the URI of the context document.<br /><br /> -and-<br /><br /> Resolve URI references in any XSLT `document()` functions.|
 ...
 The <xref:System.Xml.Xsl.XslCompiledTransform.Load*> and <xref:System.Xml.Xsl.XslCompiledTransform.Transform*> methods include overloads that take an <xref:System.Xml.XmlResolver> object as one of its arguments. If an <xref:System.Xml.XmlResolver> is not specified, a default <xref:System.Xml.XmlUrlResolver> with no credentials is used.
