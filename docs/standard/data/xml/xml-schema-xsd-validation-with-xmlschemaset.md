...
> [!NOTE]
>  The <xref:System.Xml.Schema> namespace contains extension methods that make it easy to validate an XML tree against an XSD file when using [LINQ to XML](../Topic/LINQ%20to%20XML.md). For more information on validating XML documents with LINQ to XML, see [How to: Validate Using XSD](../Topic/How%20to:%20Validate%20Using%20XSD%20\(LINQ%20to%20XML\).md).  
