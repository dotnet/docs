...
If the date and time value doesn't represent the local time or UTC, the <xref:System.DateTime.ToUniversalTime*> method will likely return an erroneous result. However, you can use the <xref:System.TimeZoneInfo.ConvertTimeToUtc*?displayProperty=nameWithType> method to convert the date and time from a specified time zone. For details on retrieving a <xref:System.TimeZoneInfo> object that represents the destination time zone, see [Finding the time zones defined on a local system](finding-the-time-zones-on-local-system.md). The following code uses the <xref:System.TimeZoneInfo.ConvertTimeToUtc*?displayProperty=nameWithType> method to convert Eastern Standard Time to UTC:

[!code-csharp[System.TimeZone2.Concepts#7](./snippets/timezone-concepts/TimeZone2Concepts.cs#7)]
[!code-vb[System.TimeZone2.Concepts#7](../../../samples/snippets/visualbasic/VS_Snippets_CLR_System/system.TimeZone2.Concepts/VB/TimeZone2Concepts.vb#7)]

The <xref:System.TimeZoneInfo.ConvertTimeToUtc*?displayProperty=nameWithType> method throws an <xref:System.ArgumentException> if the <xref:System.DateTime> object's <xref:System.DateTime.Kind> property and the time zone are mismatched. A mismatch occurs if the <xref:System.DateTime.Kind> property is <xref:System.DateTimeKind.Local?displayProperty=nameWithType> but the <xref:System.TimeZoneInfo> object doesn't represent the local time zone, or if the <xref:System.DateTime.Kind> property is <xref:System.DateTimeKind.Utc?displayProperty=nameWithType> but the <xref:System.TimeZoneInfo> object doesn't equal <xref:System.TimeZoneInfo.Utc?displayProperty=nameWithType>.
...
To convert UTC to local time, see the [Converting UTC to local time](#converting-utc-to-local-time) section that follows. To convert UTC to the time in any time zone that you designate, call the <xref:System.TimeZoneInfo.ConvertTimeFromUtc*> method. The method takes two parameters:

- The UTC to convert. This must be a <xref:System.DateTime> value whose <xref:System.DateTime.Kind> property is set to `Unspecified` or `Utc`.
...
To convert UTC to local time, call the <xref:System.DateTime.ToLocalTime*> method of the <xref:System.DateTime> object whose time you want to convert. The exact behavior of the method depends on the value of the object's <xref:System.DateTime.Kind> property, as the following table shows:
...
> [!NOTE]
> The <xref:System.TimeZone.ToLocalTime*?displayProperty=nameWithType> method behaves identically to the `DateTime.ToLocalTime` method. It takes a single parameter, which is the date and time value, to convert.

You can also convert the time in any designated time zone to local time by using the `static` (`Shared` in Visual Basic) <xref:System.TimeZoneInfo.ConvertTime*?displayProperty=nameWithType> method. This technique is discussed in the next section.
...
- <xref:System.TimeZoneInfo.ConvertTime*>
...
- <xref:System.TimeZoneInfo.ConvertTimeBySystemTimeZoneId*>
