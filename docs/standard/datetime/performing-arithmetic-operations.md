...
In this example, the [DateTimeOffset.CompareTo](xref:System.DateTimeOffset.CompareTo_System_DateTimeOffset_) method indicates that the current local time and the current UTC time are equal, and subtraction of [DateTimeOffset](xref:System.DateTimeOffset) values indicates that the difference between the two times is [TimeSpan.Zero](xref:System.TimeSpan.Zero). 
