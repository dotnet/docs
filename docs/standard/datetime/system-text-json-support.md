...
The preceding code also can be deserialized with <xref:System.Text.Json.JsonSerializer>:
...
and <xref:System.DateTimeOffset> representations. The following example shows how, when given a collection of temperatures, the average
...
`DateTime(Offset).Parse` method in your converter to read logic. This method allows you to use .NET's extensive support for parsing various
...
the extended ISO 8601-1:2019 profile. This approach is less performant than using the serializer's native implementation.

For serializing, you can use the `DateTime(Offset).ToString` method in your converter to write logic. This method allows you to write <xref:System.DateTime>
