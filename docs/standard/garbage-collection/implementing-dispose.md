...
The pattern for disposing an object, referred to as a [dispose pattern](/dotnet/standard/garbage-collection/implementing-dispose), imposes order on the lifetime of an object. The dispose pattern is used only for objects that access unmanaged resources, such as file and pipe handles, registry handles, wait handles, or pointers to blocks of unmanaged memory. This is because the garbage collector is very efficient at reclaiming unused managed objects, but it is unable to reclaim unmanaged objects.  
...
- [Dispose Pattern](/dotnet/standard/garbage-collection/implementing-dispose)
