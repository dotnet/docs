...
> [!IMPORTANT]
> When unzipping files, you must look for malicious file paths, which can escape out of the directory you unzip into. This is known as a path traversal attack.
...
## Example 3: Add a new file to an existing .zip

The following example uses the <xref:System.IO.Compression.ZipArchive> class to access an existing *.zip* file, and adds a new file to it. The new file gets compressed when you add it to the existing *.zip* file.
