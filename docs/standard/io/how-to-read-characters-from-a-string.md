...
## Example: Read characters asynchronously  
 This example reads all the characters asynchronously from a <xref:System.Windows.Controls.TextBox> control and stores them in an array. The example then asynchronously writes each letter or white-space character, followed by a line break, to a <xref:System.Windows.Controls.TextBlock> control in a WPF app.  
