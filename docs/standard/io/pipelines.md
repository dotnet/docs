...
❌ **Unexpected Unresponsive Application**

Unconditionally calling `PipeReader.AdvanceTo` with `buffer.End` in the `examined` position may result in the application stopping to respond when parsing a single message. The next call to `PipeReader.AdvanceTo` won't return until:
