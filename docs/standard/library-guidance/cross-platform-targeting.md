...
![check mark icon](../../media/check-mark.png) **DO** start with including a `netstandard2.0` target.
...
![x icon](../../media/x.png) **AVOID** including a `netstandard1.x` target.
...
![check mark icon](../../media/check-mark.png) **DO** include a `netstandard2.0` target if you require a `netstandard1.x` target.
...
![x icon](../../media/x.png) **DO NOT** include a .NET Standard target if the library relies on a platform-specific app model.
...
Sometimes you need to access framework-specific APIs from your libraries. The best way to call framework-specific APIs is using multi-targeting, which builds your project for multiple [.NET target frameworks](../frameworks.md) rather than for just one.
...
![check mark icon](../../media/check-mark.png) **CONSIDER** targeting .NET implementations in addition to .NET Standard.
...
![x icon](../../media/x.png) **AVOID** multi-targeting as well as targeting .NET Standard, if your source code is the same for all targets.
...
![check mark icon](../../media/check-mark.png) **CONSIDER** adding a target for `net461` when you're offering a `netstandard2.0` target.
...
![check mark icon](../../media/check-mark.png) **DO** use a project file's `TargetFrameworks` property when multi-targeting.
...
![check mark icon](../../media/check-mark.png) **CONSIDER** using [MSBuild.Sdk.Extras](https://github.com/onovotny/MSBuildSdkExtras) when multi-targeting for UWP and Xamarin as it greatly simplifies your project file.
...
![x icon](../../media/x.png) **DO NOT** include a Portable Class Library (PCL) target. For example, `portable-net45+win8+wpa81+wp8`.

> .NET Standard is the modern way to support cross-platform .NET libraries and replaces PCLs.

![x icon](../../media/x.png) **DO NOT** include targets for .NET platforms that are no longer supported. For example, `SL4`, `WP`.
