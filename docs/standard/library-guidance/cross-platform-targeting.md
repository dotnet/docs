...
**![no icon](../../media/no.png) AVOID** including a `netstandard1.x` target.
...
**![no icon](../../media/no.png) DO NOT** include a .NET Standard target if the library relies on a platform-specific app model.
...
**![no icon](../../media/no.png) AVOID** multi-targeting as well as targeting .NET Standard, if your source code is the same for all targets.
...
**![no icon](../../media/no.png) DO NOT** include a Portable Class Library (PCL) target. For example, `portable-net45+win8+wpa81+wp8`.

> .NET Standard is the modern way to support cross-platform .NET libraries and replaces PCLs.

**![no icon](../../media/no.png) DO NOT** include targets for .NET platforms that are no longer supported. For example, `SL4`, `WP`.
