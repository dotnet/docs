...
The following guidance is for `[DllImport]` use and must be manually followed for correct results. .NET 7+ users should consider using `[LibraryImport]` instead. A Roslyn analyzer (enabled by default) will provide the guidance required by the specific `[LibraryImport]` attributes in your code.
