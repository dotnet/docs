...
> [!NOTE]
> The C# samples in this article make use the `async` modifier on the `Main` method. That feature is available in C# 7.1 and later. Previous versions generate `CS5001` when compiling this sample code. You'll need to set the language version to C# 7.1 or newer. You can learn how to configure the language version in the article on [configure language version](../../csharp/language-reference/configure-language-version.md).
