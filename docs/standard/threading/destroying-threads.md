...
The <xref:System.Threading.Thread.Abort%2A?displayProperty=nameWithType> method [is not supported](../../core/compatibility/core-libraries/5.0/thread-abort-obsolete.md) in .NET 5 and later versions. If you need to terminate the execution of third-party code forcibly in .NET 5+, run it in the separate process and use <xref:System.Diagnostics.Process.Kill%2A?displayProperty=nameWithType>.
