...
The <xref:System.Threading.SemaphoreSlim> class represents a lightweight, fast semaphore that can be used for waiting within a single process when wait times are expected to be short—typically a few milliseconds or less. <xref:System.Threading.SemaphoreSlim> uses spinning before falling back to a kernel-based wait, so it's most efficient when the wait is brief. For longer or unpredictable wait times, use <xref:System.Threading.Semaphore?displayProperty=nameWithType> instead. <xref:System.Threading.SemaphoreSlim> relies as much as possible on synchronization primitives provided by the common language runtime (CLR). However, it also provides lazily initialized, kernel-based wait handles as necessary to support waiting on multiple semaphores. <xref:System.Threading.SemaphoreSlim> also supports the use of cancellation tokens, but it doesn't support named semaphores or the use of a wait handle for synchronization.
