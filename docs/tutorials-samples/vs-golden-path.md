...
These scripts assume that .NET Core CLI, Visual Studio Update 2, and Visual Studio tooling are installed on the machine.
...
3. Go back to packages and install `Microsoft.NETCore.Portable.Compatibility` from the Core MyGet package source (use an rc2 version). Restore packages. (*)
...
6. Go back to packages and install `Microsoft.NETCore.Portable.Compatibility` from the Core MyGet package source (use an rc2 version). Restore packages. (*)
...
3.1. Add a dependency to `"System.Runtime.Serialization.Primitives": "4.1.1-rc2-24008"`, references to `Microsoft.NETCore.Portable.Compatibility`, and add `"imports": [ "portable-net40+sl5+win8+wp8+wpa81", "dotnet" ]` to frameworks/netcoreapp1.0 in `project.json`. Restore packages. (*)
