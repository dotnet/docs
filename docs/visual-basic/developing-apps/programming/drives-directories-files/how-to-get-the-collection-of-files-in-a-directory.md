...
The overloads of the <xref:Microsoft.VisualBasic.FileIO.FileSystem.GetFiles*?displayProperty=nameWithType> method return a read-only collection of strings representing the names of the files within a directory:
...
- Use one of the <xref:Microsoft.VisualBasic.FileIO.FileSystem.GetFiles*?displayProperty=nameWithType> method overloads, supplying the name and path of the directory to search in the `directory` parameter. The following example returns all files in the directory and adds them to `ListBox1`.
