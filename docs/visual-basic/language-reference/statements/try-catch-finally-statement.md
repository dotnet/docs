...
## Iterators  
 ...
 A `Yield` statement can be inside a `Try` block. A `Try` block that contains a `Yield` statement can have `Catch` blocks, and can have a `Finally` block. See the "Try Blocks in Visual Basic" section of [Iterators](https://msdn.microsoft.com/library/dscyy5s0.aspx) for an example.  
