...
- Execute the query by applying <xref:System.Linq.Enumerable.ToList*?displayProperty=nameWithType> instead of <xref:System.Linq.Enumerable.ToArray*>.
...
- Add a while loop that performs the following steps for each task in the collection.
    ...
    3. Awaits `finishedTask`, which is returned by a call to `ProcessURLAsync`. The `finishedTask` variable is a <xref:System.Threading.Tasks.Task`1> where `TReturn` is an integer. The task is already complete, but you await it to retrieve the length of the downloaded website, as the following example shows.
