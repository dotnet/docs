...
-   You are working with code written in another language in which the given keyword is not reserved.  
 ...
 Otherwise, you should consider renaming the element if its name conflicts with a keyword. The integrated development environment (IDE) provides an easy way to do this. For more information, see [Refactoring](https://docs.microsoft.com/en-us/visualstudio/vb-ide/refactoring-vb.md).  
