---
description: "Learn more about: Operator Procedures (Visual Basic)"
...
You use the `Widening` or `Narrowing` keyword only on a type conversion operator. The operator symbol is always [CType Operator](../../../language-reference/operators/ctype-operator.md) for a type conversion operator.
