...
1. **Overload Resolution Priority.** The compiler removes any overload that has a lower <xref:System.Runtime.CompilerServices.OverloadResolutionPriorityAttribute> than the highest value. Any overload without this attribute is assigned the default value of zero.
