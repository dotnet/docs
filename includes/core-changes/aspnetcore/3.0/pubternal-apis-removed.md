...
To better maintain the public API surface of ASP.NET Core, the majority of types in `*.Internal` namespaces (referred to as "pubternal" APIs) have become truly internal. Members in these namespaces were never meant to be supported as public-facing APIs. The APIs could break in minor releases and often did. Code that used to depend on these APIs breaks when updating to ASP.NET Core 3.0.
...
These APIs are marked with the `public` access modifier and exist in `*.Internal` namespaces.

#### New behavior

These APIs are marked with the `internal` access modifier and can no longer be used.
...
* Weren't subject to our policies to prevent breaking changes.

It was determined that leaving the APIs `public` (even in the `*.Internal` namespaces) was still confusing to customers.
...
In an ASP.NET Core 3.0 project, replace the `EnableRewind` with a call to the `EnableBuffering` extension method. The request buffering feature works as it did in the past. `EnableBuffering` calls the now `internal` API.
...
All APIs in the `Microsoft.AspNetCore.*` or `Microsoft.Extensions.*` namespaces that have an `Internal` segment in the namespace name. For example:
