...
Use sentence-style capitalization. Always capitalize the first word of a heading, including the word following a colon in a title or heading (for example, "Tutorial: Predict prices using regression with ML.NET"). Don't add a colon after How to (for example, "How to sort an array").
